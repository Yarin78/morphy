...
public class VerifyGameFlags {
  ...
  private static GameMovesModel getMoves(
      MovesBase movesBase, AnnotationBase annotationBase, GameHeader gameHeader)
  ...
  private static void verifyFlag(
      int gameId,
      EnumSet<GameHeaderFlags> headerFlags,
      GameHeaderFlags headerFlag,
      boolean movesValue,
    ...
    if (headerValue != movesValue) {
      ...
      if (!suppress) {
        log.warn(
            String.format(
                "Game #%d: %s mismatch (header %s, moves %s)",
                gameId, headerFlag, headerValue, movesValue));
      }
    }
  }
  ...
  public static void verifyHeader(GameHeader header, GameMovesModel moves) {
    ...
    verifyFlag(
        id, flags, CRITICAL_POSITION, annotations.containsKey(CriticalPositionAnnotation.class));
    verifyFlag(
        id,
        flags,
    ...
    verifyFlag(
        ...
        flags,
        COMMENTARY,
        annotations.containsKey(TextAfterMoveAnnotation.class)
            || annotations.containsKey(TextBeforeMoveAnnotation.class),
        CheckFlags.SUPPRESS_FALSE_POSITIVE);
    verifyFlag(id, flags, SYMBOLS, annotations.containsKey(SymbolAnnotation.class));
    verifyFlag(
        id, flags, GRAPHICAL_SQUARES, annotations.containsKey(GraphicalSquaresAnnotation.class));
    verifyFlag(
        id, flags, GRAPHICAL_ARROWS, annotations.containsKey(GraphicalArrowsAnnotation.class));
    ...
    //        int noText = annotations.getOrDefault(TextAfterMoveAnnotation.class, 0) +
    // annotations.getOrDefault(TextBeforeMoveAnnotation.class, 0);
    ...
    if (textLength < minTextLength || textLength > maxTextLength) {
      log.warn(
          String.format(
              "Game #%d: Text annotations length was %d but expected to be in range [%d,%d]",
              id, textLength, minTextLength, maxTextLength));
    }
    //        if (noText > 0 && header.getCommentariesMagnitude() == 2) {
    //        if (noText > 0 && header.getCommentariesMagnitude() == 2) {
    //            log.info(String.format("Game #%d: No text annotations = %d, magnitude = %d, length
    // = %d", id, noText, header.getCommentariesMagnitude(), textLength));
    //        }
  ...
  private static void verifyMedals(GameHeader header, GameMovesModel moves) {
    ...
    for (Medal medal : movesMedals) {
      if (!header.getMedals().contains(medal)) {
        log.warn(
            String.format(
                "Game #%d: Moves contain medal %s which header does not have",
                header.getId(), medal));
      }
    }
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      ...
      //            int gameIds[] = new int[] { 4512477, 4753978,4855756,
      // 4904186,5140032,5140036,5140038,5355421}; // Games where annotation exists outside range
    ...
    log.info(
        String.format("%d unsupported games, %d guiding texts", noUnsupported, noGuidingTexts));
    ...
    for (GameHeaderFlags flag : GameHeaderFlags.values()) {
      ...
      if (tot > 0) {
        log.info(
            String.format(
                "%25s  Pos match: %5d    Mismatch: %3d  (false pos: %3d, false neg: %3d)",
                flag,
                noPositiveMatches[ix],
