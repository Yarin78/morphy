...
public class LoadAllGames {
  ...
  public static void main(String[] args) throws IOException, ChessBaseInvalidDataException {
    //        String fileBase =
    // "/Users/yarin/chessbasemedia/mediafiles/cbh/tmp/movedatafragmentation";
    ...
    try {
      ...
      //            int start = 4939, stop = 4939; //  From,Martin Severin-Winawer,Szymon Paris
      // Paris 1867.06.04 0-1
      //            int start = 4489434, stop = base.size();
      //            int start = 4940, stop = base.size(); //  From,Martin Severin-Winawer,Szymon
      // Paris Paris 1867.06.04 0-1
      ...
      for (int i = start; i <= stop; i++) {
        ...
        if (ofs != 0) {
          ...
          //                    log.info(String.format("#%d: Found %d annotations", i,
          // annotations.size()));
          int posNo = 0;
          //                    for (Annotations anno : annotations.values()) {
          //                        for (Annotation annotation : anno.getAll()) {
          //                            log.info(String.format("Annotation type %s found in game %d
          // after position %d", annotation.getClass().getName(), i, posNo));
        ...
        //                    }
        //                }
        //                log.info(String.format("#%d: moves offset = %d, moves size = %d,
        // annotation offset = %d, annotation size = %d",
        //                        i,
        //                        gameHeader.getMovesOffset(), movesSize,
        //                        gameHeader.getAnnotationOffset(), annotationSize));

        //                log.info(String.format("#%d: annotation offset = %d, annotation header =
        // %s", i,
        //                        gameHeader.getAnnotationOffset(),
        //                        getAnnotationHeader(annotationChannel,
        // gameHeader.getAnnotationOffset())));
      }

      //            for (int i = 0; i < 256; i++) {
      //                if (annotationCount[i] > 0) {
      //                    log.info(String.format("UnknownAnnotation type %02X occurs %d times", i,
      // annotationCount[i]));
      //                }
      //            }
    } catch (IOException e) {
