...
public class ExtractTopTournamentGames {
    ...
    public static void main(String[] args) throws IOException {
        // TODO: Iterating over all tournaments using the iterator doesn't seem to include all tournaments!?
        ...
        for (int i = 0; i < 30; i++) {
            System.out.println(String.format("Category %2d: %4d tournaments, %8d games", i, tcnt[i], gcnt[i]));
        }
        output.close();
    }
    ...
    private static void getGames(GameHeaderBase headers, TournamentEntity entity, PlayerBase players, BufferedWriter output) throws IOException {
        ...
        while (iter.hasNext() && found < entity.getCount()) {
            ...
            if (header.getTournamentId() == entity.getId()) {
                ...
                if (!header.isGuidingText()) {
                    ...
                    output.write(
                            String.format("%d;%d;%d;\"%s\";\"%s\";%d;%d;\"%s\";%d;%d;\"%s\";\"%s\";\"%s\";%d;%d;\"%s\"\n",
                                    header.getId(),
        ...
        if (found < entity.getCount()) {
            System.out.println(String.format("Only found %d games in '%s'; expected %d. Tournament marked as %s.",
                    found, entity.getTitle(), entity.getCount(), complete));
        } else {
            //System.out.println(String.format("Found all %d games in '%s' between %d and %d. Tournament marked as %s.",
