...
from cb2.entities import GameTag, Player, Source, Team, TextTitle, Tournament
...
# the titles of guiding texts live in the game_tag type. See the specification.
...
# Logically deleted entities form a linked list (see the specification). A deleted
...
# the specification). Its size depends on the number of tiebreak rules, which are a
...
# What follows the names of a player (see the specification): two ints that are
...
def _deserialize_player(entity_id, data):
    """Decode a player record as described in the specification. Raises ValueError
...
def _deserialize_source(entity_id, data):
    """Decode a source record as described in the specification."""
    try:
        record = _record_of(data)
...
def _deserialize_team(entity_id, data):
    """Decode a team record as described in the specification."""
    try:
        record = _record_of(data)
        title, offset = _read_length_prefixed_string(record, 4)
...
def _deserialize_game_tag(entity_id, data):
    """Decode a game tag record as described in the specification: a count, then that
...
def _deserialize_tournament(entity_id, data):
    """Decode a tournament record as described in the specification. Raises ValueError
    if the record doesn't have exactly that layout."""
    try:
        record = _record_of(data)
