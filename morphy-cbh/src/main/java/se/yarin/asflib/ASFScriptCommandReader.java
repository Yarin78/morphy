...
public class ASFScriptCommandReader {
  ...
  /**
   * Validates that the stream contains data in the ASF container format, and that there is a header
   * object with script commands Is called automatically by {@link #read()} if not done so
   * explicitly. May only be called once.
   *
   * @return the number of script commands
   * @throws ASFException if the input stream is not a valid ASF file
   * @throws IOException if there were an error reading the input, or if init has been called more
   *     than once
   */
