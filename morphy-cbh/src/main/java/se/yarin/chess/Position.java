...
/**
 * Represents an immutable position in a chess game, including side to move, castle rights and en
 * passant file. Enough information is contained generate all legal moves.
 *
 * <p>If the position is invalid, the behaviour of some methods are undefined.
 *
 * <p>Supports Chess960
 */
public class Position {
  ...
  private static final Stone[] startBoard =
      new Stone[] {
  ...
  public Position(
    ...
    this(
        ...
        playerToMove,
        castlesToInt(castles),
        epFile,
        chess960sp);
  }

  // IMPORTANT: For performance reason, board is not cloned. It's up to the caller to make sure not
  // to modify
  ...
  public static Position fromString(
      String boardStr, Player playerToMove, EnumSet<Castles> castles, int epFile) {
    return fromString(boardStr, playerToMove, castles, epFile, Chess960.REGULAR_CHESS_SP);
  }

  public static Position fromString(
      String boardStr, Player playerToMove, EnumSet<Castles> castles, int epFile, int chess960sp) {
  ...
  private void addPawnMove(int x1, int y1, int x2, int y2, List<Move> moveList) {
    if (y2 > 0 && y2 < 7) moveList.add(new Move(this, x1, y1, x2, y2));
    else {
  ...
  private static final int[] directionX =
      new int[] {0, 1, 0, -1, 1, 1, -1, -1, 1, 2, 2, 1, -1, -2, -2, -1};
  private static final int[] directionY =
      new int[] {1, 0, -1, 0, 1, -1, -1, 1, 2, 1, -1, -2, -2, -1, 1, 2};

  private List<Move> genericMoves(int sqi, int dirBegin, int dirEnd, boolean multi) {
    if (!stoneAt(sqi).hasPlayer(toMove)) {
      throw new IllegalArgumentException(
          "No piece at " + Chess.sqiToStr(sqi) + " with color " + toMove);
    }
    int x = Chess.sqiToCol(sqi), y = Chess.sqiToRow(sqi);
  ...
  public List<Move> generatePawnMoves(int sqi) {
    if (stoneAt(sqi) != PAWN.toStone(toMove)) {
      throw new IllegalArgumentException(
          "There is no pawn of color " + toMove + " at the square " + Chess.sqiToStr(sqi));
    }
    ArrayList<Move> moveList = new ArrayList<>();
    int x = Chess.sqiToCol(sqi), y = Chess.sqiToRow(sqi);
  ...
  List<Move> generateKingMoves(int sqi) {
    if (stoneAt(sqi) != KING.toStone(toMove)) {
      throw new IllegalArgumentException(
          "There is no king of color " + toMove + " at the square " + Chess.sqiToStr(sqi));
    }
  ...
  /**
   * Performs a move and returns the new position. This assumes that the move is pseudolegal. The
   * resulting position may thus be a board where the king can be captured in the next move.
   *
  ...
  /**
   * Generates all "pseudo legal" moves in the position. A pseudo legal move is a legal move in all
   * respects except that the king might be captured after the move has been made.
   *
  ...
  /**
   * Generates all "pseudo legal" moves in the position by specific pieces. A pseudo legal move is a
   * legal move in all respects except that the king might be captured after the move has been made.
   *
  ...
  public List<Move> generateAllLegalMoves() {
    if (allLegalMoves == null) {
      allLegalMoves =
          generateAllPseudoLegalMoves().stream()
