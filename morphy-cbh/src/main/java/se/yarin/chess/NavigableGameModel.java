 ...
 *
 * <p>Contains extra functionality for modifying the game at the current cursor position.
 */
public class NavigableGameModel {
  ...
  /**
   * Gets the current position of the cursor. There is no guarantee that the cursor points to a
   * valid node, in case the underlying moves model have been changed.
   *
  ...
  /**
   * Moves the cursor to the next position in the specified line. Line 0 is the main line, line 1
   * the first variation etc
   *
   * @return true if successful, false otherwise
   */
  ...
  /**
   * Adds a move at the current position and updates the cursor. If the position already has moves,
  ...
  /**
   * Adds a new main move at the current position and updates the cursor, The old main move becomes
   * a variation (the last one, in case other variations also exist).
   *
   * @param move the move to add
   * @throws IllegalMoveException if the move is illegal
   */
  ...
  /**
   * Inserts a move at the current position and updates the cursor, see {@link
   * se.yarin.chess.GameMovesModel.Node#insertMove(Move)} )}
   *
  ...
  /**
   * Deletes the current variation and updates the cursor to the node before the variation started.
