 ...
 * by an index. It's up to the implementation to map a valid index range. The storage also contains
 * a header where metadata about the storage can be stored. It's up to the implementation of the
 * interface to handle the logic of the items and header, serialization etc
 *
...
public interface ItemStorage<THeader, TItem> {
  ...
  /**
   * Determines the number of items in the storage based on the storage size and the size of
   * individual items
   *
   ...
   * @throws IllegalArgumentException if the index points to a place in the storage that couldn't be
   *     written to (it's up to the implementation to decide valid indexes)
   */
  void putItem(int index, @NotNull TItem item);

  /**
   * Gets a range of items from the storage If any index in the range is invalid, {@link
   * IllegalArgumentException} should be thrown, unless the storage is opened in non-strict (safe)
   * mode, in which case empty items can be returned.
   *
   * @param index the id of the first item
   * @param count the number of items to return
   * @return a list of items
   * @throws IllegalArgumentException if any index in the range points to an item outside of the
   *     storage and the storage is open in strict mode
  ...
  /**
   * Gets a range of items from the storage that matches the given filter If any index in the range
   * is invalid, {@link IllegalArgumentException} should be thrown, unless the storage is opened in
   * non-strict (safe) mode, in which case empty items can be returned.
   *
   * @param index the id of the first item
   * @param count the number of items to return
   * @param filter the filter to match items against
   * @return a list of items. Will always contain count items; non-matching items will have null
   *     values
   * @throws IllegalArgumentException if any index in the range points to an item outside of the
   *     storage and the storage is open in strict mode
   * @throws MorphyIOException if an IO error occurred when reading the data
