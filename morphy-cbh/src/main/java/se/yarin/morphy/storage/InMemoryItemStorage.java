...
public class InMemoryItemStorage<THeader, TItem> implements ItemStorage<THeader, TItem> {
  ...
  public InMemoryItemStorage(
      @Nullable DatabaseContext context, @Nullable String storageName, @NotNull THeader header) {
   ...
   *
   * @param header a header representing an empty storage
   * @param emptyItem the item to be returned when trying to read outside the index boundaries of
   *     the storage; if null, the storage is opened in strict mode
   */
    ...
    if (index < 0 || index >= items.size()) {
      if (emptyItem == null) {
        throw new IllegalArgumentException(
            String.format(
                "Tried to get item at index %d but InMemoryStorage has %d items",
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (j < 0 || j >= items.size()) {
          if (emptyItem == null) {
            throw new IllegalArgumentException(
                String.format(
                    "Tried to get item with id %d but InMemoryStorage only has %d items",
  ...
  public void putItem(int index, @NotNull TItem item) {
    ...
    if (index > items.size()) {
      throw new IllegalArgumentException(
          String.format(
              "Tried to put item with id %d but InMemoryStorage only has %d items",
              index + (oneIndexed ? 1 : 0), this.items.size()));
    }
