...
public interface ItemStorageSerializer<THeader, TItem> {
  ...
  /**
   * The actual size of the header, according to the file metadata. This may differ from the {@link
   * #serializedHeaderSize()}, in which case writing is not possible.
   *
  ...
  @NotNull
  THeader deserializeHeader(@NotNull ByteBuffer buf) throws MorphyInvalidDataException;

  /**
   * Deserialized an item and updates the buffer pointer to point to the next item in the buffer (or
   * the end of the buffer)
   */
  @NotNull
  TItem deserializeItem(int id, @NotNull ByteBuffer buf, @NotNull THeader header);
