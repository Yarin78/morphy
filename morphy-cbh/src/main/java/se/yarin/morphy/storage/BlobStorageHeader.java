...
public class BlobStorageHeader {
  ...
  public ByteBuffer serialize() {
    if (headerSize() != LEGACY_SERIALIZED_HEADER_SIZE
        && headerSize() != DEFAULT_SERIALIZED_HEADER_SIZE) {
      throw new MorphyNotSupportedException(
          String.format("Invalid header size (%d) in BlobStorageHeader", headerSize()));
    }
  ...
  public void validate(boolean strict, @NotNull String headerName, long expectedSize) {
    if (headerSize() != 10 && headerSize() != 26) {
      ...
      if (headerSize() >= 256) {
        ...
        throw new MorphyInvalidDataException(
            "Invalid header size in " + headerName + ": " + headerSize());
      }
    }

    // In later versions of the header a long version of the same value was included
    // These values can sometimes be out of sync. The 32 bit version seems to be the one that is
    // always correct.
    // However, we rely on neither of these values when appending blobs all because we use the
    // actual
    ...
    if (size() != (int) sizeLong()) {
      ...
      String msg =
          String.format("Blob storage file size values don't match (%d != %d)", size(), sizeLong());
      if (strict) {
        throw new MorphyInvalidDataException(msg);
      } else {
        log.warn(msg);
    ...
    if (wasted() != (int) wastedLong()) {
      String msg =
          String.format(
              "Blob storage wasted values don't match (%d != %d)", wasted(), wastedLong());
      if (strict) {
        throw new MorphyInvalidDataException(msg);
      } else {
        log.warn(msg);
    ...
    if (expectedSize != size()) {
      String msg =
          String.format(
              "%s: File size doesn't match size in header (%d != %d)",
              headerName, expectedSize, size());
      if (strict) {
        throw new MorphyInvalidDataException(msg);
      } else {
        log.warn(msg);
