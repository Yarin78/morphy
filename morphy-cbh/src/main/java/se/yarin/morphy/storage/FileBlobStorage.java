...
public class FileBlobStorage implements BlobStorage {
  ...
  public FileBlobStorage(
      ...
      @NotNull BlobSizeRetriever blobSizeRetriever,
  ...
  public @NotNull ByteBuffer getBlob(long offset) {
    ...
    try {
      ...
      throw new MorphyIOException(
          "Failed to get blob at offset " + offset + " in " + file.getName());
    }
  }
  ...
  public void putBlob(long offset, @NotNull ByteBuffer blob) {
    if (offset > getSize()) {
      throw new MorphyIOException(
          String.format("Tried to put blob at offset %d but file size was %d", offset, getSize()));
    }
