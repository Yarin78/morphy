...
public class GamesValidator {
  ...
  public void validateMovesAndAnnotationOffsets() throws MorphyException {
    ...
    for (GameHeader gameHeader : db.gameHeaderIndex().getAll()) { // TODO: iterable
      if (gameHeader.movesOffset() <= lastMovesOfs) {
        throw new MorphyException(
            String.format(
                "Game %d has moves at offset %d while the previous game had moves at offset %d",
      ...
      if (gameHeader.annotationOffset() > 0) {
        if (gameHeader.annotationOffset() <= lastAnnotationOfs) {
          throw new MorphyException(
              String.format(
                  "Game %d has annotations at offset %d while the last annotated game had annotations at offset %d",
      ...
      if (!gameHeader.guidingText()
          ...
          && moveOffsetStorage.getOffset(gameHeader.id()) != gameHeader.movesOffset()) {
        throw new MorphyException(
            String.format(
                "Game %d has move offset %d but booster has move offset %d",
                gameHeader.id(),
                gameHeader.movesOffset(),
                moveOffsetStorage.getOffset(gameHeader.id())));
      }
    }
    ...
    lastAnnotationOfs = 0;
    int curExtId = 0;
    for (ExtendedGameHeader extendedGameHeader :
        db.extendedGameHeaderStorage().getAll()) { // TODO: iterable
      curExtId += 1;
  ...
  public int processGames(boolean loadMoves, boolean warningAsErrors, Runnable progressCallback) {
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      for (Game game : txn.iterable()) {
        ...
        if (extendedHeader.movesOffset() != 0
            && extendedHeader.movesOffset() != header.movesOffset()
            && !moveOffsetDiffers) {
          log.warn(
              String.format(
                  "Game %d: Move offset differs between header files (%d != %d) [ignoring similar errors]",
        ...
        if (extendedHeader.annotationOffset() != 0
            && extendedHeader.annotationOffset() != header.annotationOffset()
            && !annotationOffsetDiffers) {
          log.warn(
              String.format(
                  "Game %d: Annotation offset differs between header files (%d != %d) [ignoring similar errors]",
                  header.id(), header.annotationOffset(), extendedHeader.annotationOffset()));
          annotationOffsetDiffers =
              true; // If this happens in one game, it usually happens in many games
          numWarnings += 1;
        }

        if (header.annotationOffset() > 0) {
          int annotationSize =
              db.annotationRepository().getAnnotationsBlobSize(header.annotationOffset());
          int annotationEnd = header.annotationOffset() + annotationSize;
          if (log.isDebugEnabled()) {
            log.debug(
                String.format(
                    "Game %d: Annotation [%d, %d)",
                    header.id(), header.annotationOffset(), annotationEnd));
          }
          if (header.annotationOffset() < lastAnnotationOfs) {
            if (numOverlappingAnnotations == 0) {
              log.warn(
                  String.format(
                      "Game %d has annotation data at offset %d but previous game annotation data ended at %d",
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              String.format(
                  "Game %d: Moves [%d, %d)", header.id(), header.movesOffset(), movesEnd));
        }
        if (header.movesOffset() < lastMovesOfs) {
          if (numOverlappingMoves == 0) {
            log.warn(
                String.format(
                    "Game %d has move data at offset %d but previous game move data ended at %d",
    ...
    if (moveFreeSpace != expectedUnusedMovesBytes) {
      log.warn(
          String.format(
              "Move repository header says %d unused bytes, but it actually was %d bytes",
              expectedUnusedMovesBytes, moveFreeSpace));
      numWarnings += 1;
    }
    if (moveFreeSpace != expectedUnusedMovesBytes) {
      log.warn(
          String.format(
              "Annotation repository header says %d unused bytes, but it actually was %d bytes",
              expectedUnusedAnnotationBytes, annotationFreeSpace));
      numWarnings += 1;
    }

    log.info(
        String.format(
            "%d games loaded (%d deleted, %d annotated, %d guiding texts, %d Chess960)",
            numGames, numDeleted, numAnnotated, numText, numChess960));
    if (numErrors > 0) {
      log.warn(
          String.format(
              "%d errors in the game data encountered (%d move decoding errors, %d entity references errors)",
              numErrors, numMoveDecodingErrors, numInvalidEntityReferences));
    }
    if (numOverlappingMoves > 0) {
    ...
    if (numOverlappingAnnotations > 0) {
      log.warn(
          String.format("%d games had overlapping annotation data", numOverlappingAnnotations));
    }
    if (numAnnotationGaps > 0) {
      log.info(
          String.format(
              "There were %d gaps in the annotation data (total %d bytes)",
              numAnnotationGaps, annotationFreeSpace));
    }
    if (numMoveGaps > 0) {
      log.info(
          String.format(
              "There were %d gaps in the moves data (total %d bytes)", numMoveGaps, moveFreeSpace));
    }
