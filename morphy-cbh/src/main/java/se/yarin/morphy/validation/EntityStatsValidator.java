...
public class EntityStatsValidator {
  ...
  public <T extends Entity & Comparable<T>> void processEntities(
      ...
      throws MorphyEntityIndexException {
  ...
  public <T extends Entity & Comparable<T>> void processEntities(
    ...
    try {
      for (T current : txn.iterableAscending()) {
        ...
        if (numInvalid < maxInvalid) {
          ...
          // We still need to iterate through them all so other structural checks that assumes we've
          // iterated
          // through everything doesn't fail as a side effect

          if (checks.contains(Validator.Checks.ENTITY_STATISTICS)
              || checks.contains(Validator.Checks.GAME_ENTITY_INDEX)) {
            if (!isEntityStatsValid(
                expectedGameIdsMap.get(current.id()), current, entityType, throwOnError, checks)) {
              isValidEntity = false;
            }
          }
          ...
          if (current.count() == 0) {
            ...
            String msg =
                String.format(
                    "Entity in %s base with %d (%s) has 0 count",
                    entityType, current.id(), current.toString());
            if (throwOnError) {
          ...
          if (checks.contains(Validator.Checks.ENTITY_SORT_ORDER)) {
            if (numEntities > 0) {
              if (current.compareTo(last) < 0) {
                String msg =
                    "Wrong order in %s: was (%d) '%s' < (%d) '%s' but expected opposite"
                        .formatted(entityType, last.id(), last, current.id(), current);
                if (throwOnError) {
                  throw new MorphyEntityIndexException(msg);
                }
                log.warn(msg);
    ...
    if (existingIds.size() != numEntities) {
      log.warn(
          String.format(
              "Found %d unique %s during sorted iteration, expected to find %d %s",
    ...
    if (checks.contains(Validator.Checks.ENTITY_STATISTICS)) {
      if (existingIds.size() != expectedGameIdsMap.size()) {
        log.warn(
            String.format(
                "Found %d unique %s during sorted iteration, but has statistics for %d %s",
                existingIds.size(),
                entityType.namePlural(),
                expectedGameIdsMap.size(),
                entityType.namePlural()));
      }
    ...
    if (numEntities != entityIndex.count()) {
      ...
      log.debug(
          String.format(
              "Iterated over %d %s in ascending order, but header said there were %d %s",
              numEntities, entityType.namePlural(), entityIndex.count(), entityType.namePlural()));
    }
    if (checks.contains(Validator.Checks.ENTITY_SORT_ORDER)) {
      if (numEqual > 0) {
        log.info(
            String.format("%d %s with the same key as the previous entity", numEqual, entityType));
      }
    }
    if (entityIndex.capacity() > numEntities) {
      log.debug(
          String.format(
              "Database has additional capacity for %d %s",
    ...
    for (Integer id : entityIndex.getDeletedEntityIds()) {
      if (existingIds.contains(id)) {
        log.warn(
            String.format(
                "Entity id %d was found both when iterating and among deleted nodes", id));
      }
      if (checks.contains(Validator.Checks.GAME_ENTITY_INDEX)) {
        ...
        if (gameEntityIndex != null) {
          ...
          if (gameIds.size() != 0) {
            ...
            log.warn(
                String.format(
                    "Deleted %s entity id %d has %d game references in the Game Entity Index",
    ...
    if (existingIds.size() > 0) {
      if (existingIds.first() != 0) {
        log.warn(
            String.format("First id in %s was %d, expected 0", entityType, existingIds.first()));
      }
      if (existingIds.last() != entityIndex.capacity() - 1) {
        log.warn(
            String.format(
                "Last id in %s was %d, expected %d",
                entityType, existingIds.last(), entityIndex.count() - 1));
      }
      if (existingIds.size() != entityIndex.capacity()) {
        log.warn(
            String.format(
                "Discovered %d entities in %s, but capacity is %d",
  ...
  private <T extends Entity & Comparable<T>> boolean isEntityStatsValid(
    ...
    if (expectedGameIds == null) {
      String msg =
          String.format(
              "Entity in %s base with id %d (%s) occurs in 0 games but stats says %d games and first game %d",
    ...
    if (checks.contains(Validator.Checks.ENTITY_STATISTICS)) {
      if (expectedGameIds.size() != current.count()) {
        String msg =
            String.format(
                "Entity in %s index with id %d (%s) has %d games but entity count says %d",
                entityType.nameSingular(),
                current.id(),
                current,
                expectedGameIds.size(),
                current.count());
        if (throwOnError) {
          throw new MorphyEntityIndexException(msg);
        }
      ...
      if (expectedGameIds.get(0) != current.firstGameId()) {
        String msg =
            String.format(
                "Entity in %s index with id %d (%s) first game is %d but stats says %d",
                entityType.nameSingular(),
                current.id(),
                current,
                expectedGameIds.get(0),
                current.firstGameId());
        if (throwOnError) {
          throw new MorphyEntityIndexException(msg);
        }
    ...
    if (checks.contains(Validator.Checks.GAME_ENTITY_INDEX)) {
      ...
      if (gameEntityIndex != null) {
        ...
        if (expectedGameIds.size() != indexGameIds.size()
            || !expectedGameIds.equals(indexGameIds)) {
          if (new HashSet<>(expectedGameIds).equals(new HashSet<>(indexGameIds))) {
            ...
            String msg =
                String.format(
                    "%s entity with id %d (%s) was duplicated in a game which was not reflected in the GameEntityIndex (no further warnings of this type will be logged)",
            ...
            String msg =
                String.format(
                    "%s entity with id %d (%s) mismatched in GameEntityIndex",
                    entityType.nameSingularCapitalized(), current.id(), current);
            if (throwOnError) {
              throw new MorphyEntityIndexException(msg);
            }
  ...
  public void validateEntityStatistics(boolean throwOnError) throws MorphyEntityIndexException {
    ...
    processEntities(
        EntityType.TOURNAMENT, database.tournamentIndex(), stats.tournaments, throwOnError);
    processEntities(
        EntityType.ANNOTATOR, database.annotatorIndex(), stats.annotators, throwOnError);
