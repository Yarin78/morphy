...
public class Validator {
  ...
  public enum Checks {
    ...
    // Check Player, Tournament, Annotator, Source, Team and Game Tag entity types (statistics,
    // sorting)
    ENTITY_PLAYERS,
    ENTITY_TOURNAMENTS,
    ENTITY_ANNOTATORS,
  ...
  private static class EntityTypeCheck {
    ...
    public EntityTypeCheck(
        EntityType entityType, EntityIndex<?> index, Map<Integer, List<Integer>> statMap) {
  ...
  /**
   * Performs a series of validation and integrity checks on a database Problems are logged using
   * the logging framework at ERROR or WARN level. INFO and DEBUG messages are used to log various
   * statistics
   *
  ...
  public void validate(
    ...
    if (checks.contains(Checks.ENTITY_PLAYERS)) {
      entityTypeCheck.add(
          new EntityTypeCheck(EntityType.PLAYER, db.playerIndex(), entityStats.players));
    }
    if (checks.contains(Checks.ENTITY_TOURNAMENTS)) {
      entityTypeCheck.add(
          new EntityTypeCheck(
              EntityType.TOURNAMENT, db.tournamentIndex(), entityStats.tournaments));
    }
    if (checks.contains(Checks.ENTITY_ANNOTATORS)) {
      entityTypeCheck.add(
          new EntityTypeCheck(EntityType.ANNOTATOR, db.annotatorIndex(), entityStats.annotators));
    }
    if (checks.contains(Checks.ENTITY_SOURCES)) {
      entityTypeCheck.add(
          new EntityTypeCheck(EntityType.SOURCE, db.sourceIndex(), entityStats.sources));
    }
    ...
    if (checks.contains(Checks.ENTITY_GAME_TAGS)) {
      entityTypeCheck.add(
          new EntityTypeCheck(EntityType.GAME_TAG, db.gameTagIndex(), entityStats.gameTags));
    }
    ...
    if (entityTypeCheck.size() > 0) {
      if (checks.contains(Checks.ENTITY_DB_INTEGRITY)) {
        try (ProgressTracker progressTracker =
            trackerFactory.create("File structure", entityTypeCheck.size())) {
          for (EntityTypeCheck typeCheck : entityTypeCheck) {
            try {
              typeCheck.getIndex().validateStructure();
            } catch (MorphyEntityIndexException e) {
              log.error(
                  "Error validating entity integrity for "
                      + typeCheck.entityType.namePlural()
                      + ": "
                      + e.getMessage());
      ...
      if (checks.contains(Checks.ENTITY_STATISTICS) || checks.contains(Checks.GAME_ENTITY_INDEX)) {
        try (ProgressTracker progressTracker =
            trackerFactory.create("Entity stats", db.gameHeaderIndex().count())) {
      ...
      for (EntityTypeCheck typeCheck : entityTypeCheck) {
        ...
        try (ProgressTracker progressTracker =
            trackerFactory.create(
                entityType.namePluralCapitalized(), typeCheck.getIndex().count())) {
          try {
            ...
            entityStatsValidator.processEntities(
                entityType,
                typeCheck.getIndex(),
                typeCheck.getStatMap(),
                progressTracker::step,
                checks,
                MAX_INVALID_ENTITIES,
                throwOnWarning);
          } catch (Exception e) {
            log.error(
                String.format(
                    "Error processing %s entities: %s", entityType.nameSingular(), e.getMessage()));
            hasCriticalErrors = true;
          }
        }
      }
    ...
    if (checks.contains(Checks.GAME_ENTITY_INDEX)) {
      ...
      if (gameEntityIndexSecondary != null
          && validateGameEntityIndexBlocks(db, gameEntityIndexSecondary)) {
        hasCriticalErrors = true;
      }
    }
    ...
    if (checks.contains(Checks.GAMES)) {
      ...
      try (ProgressTracker progressTracker =
          trackerFactory.create("Games", db.gameHeaderIndex().count())) {
        try {
          int numErrors =
              gamesValidator.processGames(
                  checks.contains(Checks.GAMES_LOAD), throwOnWarning, progressTracker::step);
          if (numErrors > 0) {
            hasCriticalErrors = true;
          }
        } catch (Exception e) {
  ...
  private boolean validateGameEntityIndexBlocks(
    ...
    try {
      ...
      for (EntityType entityType : gameEntityIndex.entityTypes()) {
        Set<Integer> usedBlockIds =
            gameEntityIndex.getUsedBlockIds(
                entityType, database.entityIndex(entityType).capacity());
      ...
      if (cibNumBlocks != cibBlocks.size() || maxCibBlockIndex != cibNumBlocks - 1) {
        throw new IllegalStateException(
            "There are blocks unaccounted for in the game entity index");
      }
