...
public class GameTableScan extends QueryOperator<Game> {
  ...
  public GameTableScan(
      @NotNull QueryContext queryContext,
      @Nullable GameFilter gameFilter,
  ...
  public void estimateOperatorCost(@NotNull ImmutableOperatorCost.Builder operatorCost) {
    ...
    long estimateRows =
        OperatorCost.capRowEstimate((int) Math.round(numScannedGames * matchingRatio));

    long pageReads =
        Math.round(
            (context().database().gameHeaderIndex().numDiskPages()
    ...
    operatorCost
        ...
        .estimateDeserializations(
            estimateRows * 2) // Non-matching rows will mostly be caught non-deserialized
