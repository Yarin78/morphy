...
/**
 * Scans an entity base in ID order from a given ID. Optionally filters out entities on a filter
 * predicate.
 */
public class EntityTableScan<T extends Entity & Comparable<T>> extends QueryOperator<T> {
  ...
  public void estimateOperatorCost(@NotNull ImmutableOperatorCost.Builder operatorCost) {
    ...
    long estimateRows =
        OperatorCost.capRowEstimate((int) Math.round(numScannedEntities * matchingRatio));

    long pageReads =
        Math.round(context().database().entityIndex(entityType).numDiskPages() * scanRatio);
