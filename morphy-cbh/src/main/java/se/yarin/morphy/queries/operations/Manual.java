...
public class Manual<T extends IdObject> extends QueryOperator<T> {
  ...
  protected void estimateOperatorCost(ImmutableOperatorCost.@NotNull Builder operatorCost) {
    operatorCost.estimateRows(data.size()).estimatePageReads(0).estimateDeserializations(0);
  }

  @Override
  public String toString() {
    String commaList =
        data.stream().map(data -> Integer.toString(data.id())).collect(Collectors.joining(", "));
