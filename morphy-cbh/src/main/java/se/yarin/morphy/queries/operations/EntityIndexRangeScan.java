...
public class EntityIndexRangeScan<T extends Entity & Comparable<T>> extends QueryOperator<T> {
  ...
  public EntityIndexRangeScan(
    ...
    this.txn =
        (EntityIndexReadTransaction<T>) queryContext.transaction().entityTransaction(entityType);
  ...
  public void estimateOperatorCost(@NotNull ImmutableOperatorCost.Builder operatorCost) {
    long scanCount =
        context()
            .queryPlanner()
            .entityRangeEstimate(
                !reverse ? rangeStart : rangeEnd, !reverse ? rangeEnd : rangeStart, null);
    long matchCount =
        context()
            .queryPlanner()
            .entityRangeEstimate(
                !reverse ? rangeStart : rangeEnd, !reverse ? rangeEnd : rangeStart, entityFilter);
  ...
  public String toString() {
    ...
    return entityType.nameSingularCapitalized()
        + "IndexRangeScan("
        + String.join(", ", params)
        + ")";
  }
