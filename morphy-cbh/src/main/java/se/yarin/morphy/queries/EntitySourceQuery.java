...
public class EntitySourceQuery<T extends IdObject> implements SourceQuery<T> {
    ...
    private EntitySourceQuery(@NotNull QueryOperator<T> operator,
                              boolean optional,
    ...
    public static <T extends IdObject> EntitySourceQuery<T> fromQueryOperator(@NotNull QueryOperator<T> playerQueryOperator, boolean optional, @NotNull List<EntityFilter<T>> filtersCovered) {
        return new EntitySourceQuery<>(playerQueryOperator, optional, filtersCovered);
    }

    public static <T extends IdObject> EntitySourceQuery<T> join(@NotNull EntitySourceQuery<T> left, @NotNull EntitySourceQuery<T> right) {
        ...
        if (left.operator.sortOrder().isSameOrStronger(QuerySortOrder.byId()) && right.operator.sortOrder().isSameOrStronger(QuerySortOrder.byId())) {
            return new EntitySourceQuery<T>(new MergeJoin<>(left.context(), left.operator, right.operator), false, coveredFilters);
        }
        return new EntitySourceQuery<T>(new HashJoin<>(left.context(), left.operator, right.operator), false, coveredFilters);
    }
