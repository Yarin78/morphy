...
public class QueryPlanner {
  ...
  public <T extends Entity & Comparable<T>> List<T> sampleEntities(
  ...
  public void updateStatistics() {
    ...
    try (var txn = new DatabaseReadTransaction(database)) {
      ...
      gamesRatingDistribution =
          new IntBucketDistribution(
              3000, games.stream().flatMap(game -> Stream.of(game.whiteElo(), game.blackElo())));

      playerLastNameDistribution = new StringDistribution(players.stream().map(Player::lastName));

      tournamentCategoryDistribution =
          new IntBucketDistribution(30, tournaments.stream().map(Tournament::category));
      tournamentYearDistribution =
          new IntBucketDistribution(
   ...
   * @return ratio between 0.0 and 1.0 of number of games expected to pass the filter, 0.0 being
   *     none and 1.0 being all.
   */
  ...
  public <T extends Entity & Comparable<T>> double entityFilterEstimate(
      @Nullable EntityFilter<T> entityFilter, EntityType entityType) {
    // TODO
   ...
   */
  public long playerRangeEstimate(
      @Nullable Player rangeStart,
      @Nullable Player rangeEnd,
      @Nullable EntityFilter<Player> playerFilter) {
    double ratio =
        playerLastNameDistribution.ratioBetween(
            rangeStart == null ? "" : rangeStart.lastName(),
            rangeEnd == null ? "zzz" : rangeEnd.lastName());
    return Math.max(1, Math.round(database.playerIndex().count() * ratio));
  }

  public long tournamentRangeEstimate(
      @Nullable Tournament rangeStart,
      @Nullable Tournament rangeEnd,
      @Nullable EntityFilter<Tournament> tournamentFilter) {
    return 0;
  }

  public <T extends Entity & Comparable<T>> long entityRangeEstimate(
      @Nullable T rangeStart, @Nullable T rangeEnd, @Nullable EntityFilter<T> entityFilter) {
    // TODO: Make generic
    return 1;
  }
  ...
  public long estimateGameEntityIndexPageReads(@NotNull EntityType entityType, long count) {
    ...
    // We assume here that on average, all games associated with an entity is in the same block in
    // the cib file
    return estimateUniquePages(citPages, count) + estimateUniquePages(cibPages, count);
  }

  public @NotNull <T extends IdObject> QueryOperator<T> selectBestQueryPlan(
  ...
  public List<QueryOperator<Game>> getGameQueryPlans(
    ...
    // Each such source yields a stream of either gameId's or Game records (in ascending order, no
    // duplicates).
    // The sourced are joined using a Merge join.
    // If needed a Game lookup is then done, and then the game and entity filters are applied in
    // some order.
    ...
    // Entity filters can sometimes be sources. An Entity filter can either be a perfect source
    // filter
    // (no need to apply the filter later), or only a partial source filter (the entity filter
    // needs to be applied anyway)
    // A full Game table scan is always an optional source (and needed if there are no other
    // sources)
    ...
    for (List<GameSourceQuery> sourceCombination : sourceCombinations(sources)) {
      ...
      List<GameSourceQuery> sorted =
          sourceCombination.stream()
              .sorted(Comparator.comparingLong(GameSourceQuery::estimateRows))
      ...
      // Ensure that full data exists in case we need to do additional filtering/sorting, or full
      // data is required
      ...
      if ((!gameOperator.hasFullData() && fullDataRequired) || !filtersLeft.isEmpty()) {
        ...
        gameOperator =
            new GameLookup(
                context, current.gameOperator(), CombinedGameFilter.combine(filtersLeft));
      }
      ...
      for (var entityJoinPermutation : generatePermutations(entityJoinsLeft)) {
        ...
        for (var entityJoin : entityJoinPermutation) {
          if (entityJoin.isSimpleJoin()) {
            ...
            var entityQueryOperator =
                selectBestQueryPlan(getEntityQueryPlans(context, entityJoin.entityQuery(), false));
            currentGameOperator =
                new GameEntityHashJoin(
                    context,
                    currentGameOperator,
                    entityJoin.getEntityType(),
                    entityQueryOperator,
                    entityJoin.joinCondition());
          }
        }
        candidateQueryPlans.add(
            currentGameOperator.sortedAndDistinct(sortOrder, gameQuery.limit()));
      }
    }
  ...
  List<GameSourceQuery> getGameQuerySources(
      @NotNull QueryContext context, @NotNull GameQuery gameQuery) {
    List<GameSourceQuery> sources = new ArrayList<>();

    // TODO: Investigate if a full GameTableScan is ever the right thing in case any other source is
    // available
    // Any entity index + game lookup should almost always be better
    sources.add(
        GameSourceQuery.fromGameQueryOperator(
            new GameTableScan(context, CombinedGameFilter.combine(gameQuery.gameFilters())),
            true,
            gameQuery.gameFilters(),
            List.of()));

    for (GameEntityJoin<?> entityJoin : gameQuery.entityJoins()) {
      QueryOperator<?> entityQueryPlan =
          selectBestQueryPlan(getEntityQueryPlans(context, entityJoin.entityQuery(), false));
      QueryOperator<Game> gameOp =
          new GameIdsByEntities<>(context, entityQueryPlan, entityJoin.getEntityType());
      // If joining with e.g. White players only, we still need to filter on this later on as the
      // index will return
      // games where the player is either White or Black
      boolean coveredJoin =
          entityJoin.joinCondition() == null
              || entityJoin.joinCondition() == GameEntityJoinCondition.ANY;
      sources.add(
          GameSourceQuery.fromGameQueryOperator(
              gameOp.sortedAndDistinct(),
              true,
              List.of(),
              coveredJoin ? List.of(entityJoin) : List.of()));
    }
      ...
      if (gameFilter instanceof GameEntityFilter<?> entityFilter) {
        QueryOperator<Game> gameOp =
            new GameIdsByEntities<>(
                context,
                new Manual<>(context, Set.copyOf(entityFilter.entityIds())),
                entityFilter.entityType());
        boolean coveredFilter = entityFilter.matchCondition() == GameEntityJoinCondition.ANY;
        sources.add(
            GameSourceQuery.fromGameQueryOperator(
                gameOp.sortedAndDistinct(QuerySortOrder.byId(), 0),
                true,
                coveredFilter ? List.of(gameFilter) : List.of(),
                List.of()));
      }
    }
    ...
    for (int i = 0; i < (1 << n); i++) {
      ...
      for (int j = 0; j < n; j++) {
        if (((1 << j) & i) > 0) {
          ...
          if (!seenFiltersCovered.add(sources.get(j).filtersCovered())
              && sources.get(j).isOptional()) {
            // Unnecessary to have this optional source since it's filters were already covered by
            // another source
            valid = false;
          }
        } else {
  ...
  public <T extends Entity & Comparable<T>> List<QueryOperator<T>> getEntityQueryPlans(
      @NotNull QueryContext context, @NotNull EntityQuery<T> entityQuery, boolean fullData) {
    ...
    for (List<EntitySourceQuery<T>> sourceCombination : sourceCombinations(sources)) {
      // Join the sources starting with the one returning least amount of expected rows
      List<EntitySourceQuery<T>> sortedByEstimatedRows =
          sourceCombination.stream()
              .sorted(Comparator.comparingLong(EntitySourceQuery::estimateRows))
              .collect(Collectors.toList());

      candidateQueryPlans.add(
          getEntityFinalQueryOperator(entityQuery, sortedByEstimatedRows, fullData));

      if (!entityQuery.sortOrder().isNone()) {
        // We also want to check if one of the source queries already has the data in the right
        // order.
        // If so, by starting with that one and the applying the other sources in order we might end
        // up with
        ...
        for (int i = 1; i < sortedByEstimatedRows.size(); i++) { // i=0 is already covered above
          ...
          if (sourceQuery.operator().sortOrder().isSameOrStronger(entityQuery.sortOrder())) {
            ...
            candidateQueryPlans.add(
                getEntityFinalQueryOperator(entityQuery, alternateOrder, fullData));
          }
        }
      }
    }
  ...
  <T extends Entity & Comparable<T>> List<EntitySourceQuery<T>> getEntitySources(
      @NotNull QueryContext context, @NotNull EntityQuery<T> entityQuery) {
    ...
    for (EntityFilter<T> filter : entityQuery.filters()) {
      if (filter instanceof ManualFilter<T> manualFilter) {
        ...
        if (startId == null || minId > startId) startId = minId;
        if (endId == null || maxId < endId) endId = maxId;
      }
      if (filter instanceof EntityIndexFilter<T> indexFilter) {
        T p = indexFilter.start();
        if (p != null && (startEntity == null || p.compareTo(startEntity) > 0)) startEntity = p;
        p = indexFilter.end();
        if (p != null && (endEntity == null || p.compareTo(endEntity) < 0)) endEntity = p;
      }
    }

    sources.add(
        EntitySourceQuery.fromQueryOperator(
            new EntityTableScan<>(
                context, entityQuery.entityType(), combinedFilter, startId, endId),
            true,
            entityQuery.filters()));

    boolean reverseNameOrder =
        entityQuery
            .sortOrder()
            .isSameOrStronger(
                (QuerySortOrder<T>)
                    QuerySortOrder.byEntityDefaultIndex(entityQuery.entityType(), true));
    sources.add(
        EntitySourceQuery.fromQueryOperator(
    ...
    if (gameQuery != null) {
      QueryOperator<Game> gameQueryPlan =
          selectBestQueryPlan(getGameQueryPlans(context, gameQuery, true));
      GameEntityJoinCondition joinCondition = entityQuery.joinCondition();
      QueryOperator<T> complexEntityQuery =
          new Distinct<>(
              context,
              new Sort<>(
                  context,
                  new EntityIdsByGames<T>(
                      context, entityQuery.entityType(), gameQueryPlan, joinCondition)));
  ...
  <T extends Entity & Comparable<T>> QueryOperator<T> getEntityFinalQueryOperator(
      @NotNull EntityQuery<T> entityQuery,
      @NotNull List<EntitySourceQuery<T>> sources,
      boolean fullData) {
    boolean fullDataRequired = fullData;
