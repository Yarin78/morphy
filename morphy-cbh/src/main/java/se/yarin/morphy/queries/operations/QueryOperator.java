...
public abstract class QueryOperator<T extends IdObject> {
  ...
  protected Stream<MetricsKey> streamMetricsKeys() {
    Stream<MetricsKey> metricsKeys =
        metricProviders().stream()
  ...
  protected void setOperatorActualMetrics(
      @NotNull MetricsRepository metricsRepository, Set<MetricsKey> duplicateKeys) {
    for (QueryOperator<?> source : sources()) {
    ...
    for (MetricsProvider metricProvider : metricProviders()) {
      List<MetricsKey> metricKeys =
          metricProvider.getMetricsKeys().stream().distinct().collect(Collectors.toList());
  ...
  /**
   * Gets the cost of the single query operator. If the query hasn't yet been executed, only the
   * estimate will be set. After execution, if profiling was enabled, actual cost will be included
   *
  ...
  /**
   * Gets a summary of the query cost for the entire query. If it hasn't yet been executed, only the
   * estimate will be set. After execution, if profiling was enabled, actual cost will be included.
   *
  ...
  public final QueryCost getQueryCost() {
    ...
    if (queryMetrics != null) {
      for (@NotNull
      ItemMetrics itemMetrics : queryMetrics.getMetricsByType(ItemMetrics.class).values()) {
        totalActualDeserializations += itemMetrics.deserializations();
      }
      for (@NotNull
      FileMetrics fileMetrics : queryMetrics.getMetricsByType(FileMetrics.class).values()) {
  ...
  public String debugString(boolean includeCost) {
    ...
    if (includeCost) {
      ...
      sb.append(
          String.format(
              " {data: %b, estimate: {rows=%d, deser=%d, pageReads=%d}, actual%s: {rows=%d, deser=%d, physicalPageReads=%d, logicalPageReads=%d}}",
              hasFullData,
              cost.estimateRows(),
              ...
              cost.actualRows(),
              cost.actualDeserializations(),
              cost.actualPhysicalPageReads(),
              cost.actualLogicalPageReads()));
    }
    sb.append("\n");
    for (QueryOperator<?> source : sources()) {
