...
public class QueryContext {
    ...
    public <T extends Entity & Comparable<T>> @NotNull EntityIndex<T> entityIndex(@NotNull EntityType entityType) {
        return (EntityIndex<T>) txn.database().entityIndex(entityType);
    }

    public @NotNull Database database() { return txn.database(); }

    public @NotNull QueryPlanner queryPlanner() { return database().queryPlanner(); }
