...
public class GameEventStorage
    implements ItemStorageSerializer<GameEventStorage.Prolog, GameEvents> {
  ...
  public GameEventStorage(@Nullable DatabaseContext context) {
    this(
        new InMemoryItemStorage<>(
            context, "GameEvents", GameEventStorage.Prolog.empty(), null, true),
        context);
  }

  private GameEventStorage(
      @NotNull ItemStorage<GameEventStorage.Prolog, GameEvents> storage,
  ...
  private GameEventStorage(
    ...
    this.itemMetricsRef = ItemMetrics.register(this.context.instrumentation(), "GameEvents");
    this.storage =
        new FileItemStorage<>(file, this.context, "GameEvents", this, Prolog.empty(), options);

    if (options.contains(WRITE)) {
      if (storage.getHeader().version() != Prolog.DEFAULT_VERSION) {
        throw new MorphyNotSupportedException(
            String.format(
                "Unsupported version of GameEvent storage (%d != %d)",
      ...
      if (storage.getHeader().serializedItemSize() != Prolog.DEFAULT_SERIALIZED_ITEM_SIZE) {
        ...
        throw new MorphyNotSupportedException(
            String.format(
                "Size of game event record was %d but expected %d; writing not possible.",
  ...
  public static GameEventStorage open(@NotNull File file, @Nullable DatabaseContext context)
      throws IOException {
    return open(file, DatabaseMode.READ_WRITE, context);
  }

  public static GameEventStorage open(
  ...
  @Override
  public @NotNull GameEventStorage.Prolog deserializeHeader(@NotNull ByteBuffer buf)
    ...
    for (int i = 0; i < 11; i++) {
      ...
      if (fillerInt != 0) {
        log.warn(
            String.format(
                "Unused prolog bytes in GameEventStorage was not 0 (int %d = %d)", i, fillerInt));
      }
    }
    return prolog;
  }
  ...
  public void serializeItem(
      ...
      @NotNull GameEventStorage.Prolog prolog) {
