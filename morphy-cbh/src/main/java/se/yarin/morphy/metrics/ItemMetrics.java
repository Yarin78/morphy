...
public class ItemMetrics implements Metrics {
    ...
    public static @NotNull ItemMetrics get(@NotNull MetricsRepository metricsRepository, @NotNull String name) {
    ...
    public String formatTableRow() {
        return String.format("%-15s %9d %9d %9d %9d", name, gets + getRaws, puts, deserializations, serializations);
    }

    public boolean isEmpty(int threshold) {
        return gets + getRaws <= threshold && puts <= threshold && deserializations <= threshold && serializations <= threshold;
    }
}
