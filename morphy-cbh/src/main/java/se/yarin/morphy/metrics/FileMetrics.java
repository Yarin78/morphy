...
public class FileMetrics implements Metrics {
    ...
    public static MetricsRef<FileMetrics> register(@NotNull Instrumentation instrumentation, @NotNull Path path) {
    ...
    public static MetricsRef<FileMetrics> register(@NotNull Instrumentation instrumentation, @NotNull String name) {
    ...
    public String formatTableRow() {
        return String.format("%-8s %7d %7d %7d", fileName, physicalPageReads, logicalPageReads, pageWrites);
    }

    @Override
    public boolean isEmpty(int threshold) {
        return physicalPageReads <= threshold && logicalPageReads <= threshold && pageWrites <= threshold;
    }
}
