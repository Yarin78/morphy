...
public class PlayerNameFilter implements EntityIndexFilter<Player> {
  ...
  private boolean matches(@NotNull String playerName, @NotNull String searchName) {
    if (exactMatch) {
      return caseSensitive
          ? playerName.equals(searchName)
          : playerName.equalsIgnoreCase(searchName);
    }
    return caseSensitive
        ? playerName.startsWith(searchName)
        : playerName.toLowerCase().startsWith(searchName);
  }
  ...
  public String toString() {
    ...
    if (exactMatch) {
      ...
      return "%s like '%s%%' and %s like '%s%%'"
          .formatted(lastNameStr, lastName, firstNameStr, firstName);
    }
  }
