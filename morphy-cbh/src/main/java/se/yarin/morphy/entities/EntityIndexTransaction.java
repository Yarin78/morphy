...
public abstract class EntityIndexTransaction<T extends Entity & Comparable<T>>
  ...
  /**
   * Returns a NodePath to the first node which does not compare less than entity, or NodePath.end
   * if no such node exists. If nodes exists with that compares equally to entity, the first of
  ...
  /**
   * Returns a NodePath to the first node which compares greater than entity, or NodePath.end if no
   * such node exists.
   */
  ...
  /**
   * A path in the Entity node tree. This class is immutable. When using TreePath in iteration, it
   * should never be null; use TreePath.end() to mark end of iteration.
   */
