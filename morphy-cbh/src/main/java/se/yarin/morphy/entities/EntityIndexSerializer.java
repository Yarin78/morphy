...
public class EntityIndexSerializer implements ItemStorageSerializer<EntityIndexHeader, EntityNode> {
  ...
  public @NotNull EntityIndexHeader deserializeHeader(@NotNull ByteBuffer buf)
    ...
    if (rootEntityId >= capacity) {
      throw new MorphyInvalidDataException(
          String.format("Root entity is %d but capacity %d", rootEntityId, capacity));
    }
    ...
    if (constant != EntityIndexHeader.MAGIC_CONSTANT) {
      ...
      throw new MorphyInvalidDataException(
          String.format(
              "Invalid magic constant in header; was %d, expected %d",
    ...
    if (serializedEntitySize != recordSize) {
      throw new MorphyInvalidDataException(
          String.format(
              "Invalid record size in entity index; was %d, expected %d",
              serializedEntitySize, recordSize));
    }
  ...
  @Override
  public @NotNull EntityNode deserializeItem(
      int id, @NotNull ByteBuffer buf, @NotNull EntityIndexHeader header) {
    ...
    return new EntityNode(
        id, leftEntityId, rightEntityId, heightDif, count, firstGameId, serializedEntity);
  }

  @Override
  public void serializeItem(
      @NotNull EntityNode node, @NotNull ByteBuffer buf, @NotNull EntityIndexHeader header) {
