...
public abstract class EntityIndex<T extends Entity & Comparable<T>> implements MetricsProvider {
  ...
  @NotNull
  MetricsRef<ItemMetrics> itemMetricsRef() {
  ...
  protected EntityIndex(
    ...
    this.itemMetricsRef =
        ItemMetrics.register(this.context.instrumentation(), entityType.nameSingularCapitalized());
  ...
  /**
   * Gets an entity node in the index If the index is invalid, either an empty (non-null) item is
   * returned, or {@link IllegalArgumentException} is thrown depending on the open option.
   *
   * @param id the id of the entity
   * @return an entity node
   * @throws IllegalArgumentException if the index points to an item outside of the storage and the
   *     storage is open in strict mode
  ...
  protected @NotNull T resolveEntity(@NotNull EntityNode node) throws IllegalArgumentException {
    ...
    return deserialize(
        node.getId(), node.getGameCount(), node.getFirstGameId(), node.getSerializedEntity());
  }
  ...
  /**
   * Gets an entity by id. If the id is invalid (outside of the index, or the item has been
   * deleted), empty (non-null) item is returned or {@link IllegalArgumentException} is thrown,
   * depending on the OpenOption of the index.
   *
   * @param id the id of the entity
   * @return the entity
   */
  ...
  /**
   * Gets the underlying raw data for an entity. For debugging purposes only.
   *
  ...
  /**
   * Gets the entity after the given one in the index according to the default sort order. The given
   * entity doesn't have to exist, it can be a search key as well.
   *
  ...
  /**
   * Gets the entity before the given one in the index according to the default sort order. The
   * given entity doesn't have to exist, it can be a search key as well.
   *
   * @param entity an entity or an entity key
  ...
  /**
   * Updates an existing entity in the index The entity to update is determined by the key; use
   * {@link #put(int, T)} to update the key fields of an entity
   *
  ...
  protected void copyEntities(@NotNull EntityIndex<T> targetIndex) {
    ...
    ImmutableEntityIndexHeader newHeader =
        ImmutableEntityIndexHeader.copyOf(storageHeader())
  ...
  public void validateStructure() throws MorphyEntityIndexException {
    if (storageHeader().rootNodeId() == -1) {
      ...
      throw new MorphyEntityIndexException(
          String.format(
              "Header says there are %d entities in the storage but the root points to no entity.",
              storageHeader().numEntities()));
    }
  ...
  /**
   * Gets a list of all node ids that are marked as deleted and can be reused. For internal use
   * only.
   *
  ...
  private @NotNull ValidationResult validate(
      int entityId, @Nullable T min, @Nullable T max, int depth) throws MorphyEntityIndexException {
    if (depth > 40) {
      throw new MorphyEntityIndexException(
          "Infinite loop when verifying storage structure for entity " + entityType.nameSingular());
    }
    ...
    if (node.isDeleted()) {
      throw new MorphyEntityIndexException(
          String.format(
              "Reached deleted %s entity %d when validating the storage structure.",
              entityType.nameSingular(), entityId));
    }
    if ((min != null && min.compareTo(entity) > 0) || (max != null && max.compareTo(entity) < 0)) {
      throw new MorphyEntityIndexException(
          String.format(
              "%s entity %d out of order when validating the storage structure",
              entityType, entityId));
    }
    ...
    if (rightHeight - leftHeight != node.getBalance()) {
      throw new MorphyEntityIndexException(
          String.format(
              "Height difference at node %d was %d but node data says it should be %d (entity type %s)",
              node.getId(),
    ...
    if (Math.abs(leftHeight - rightHeight) > 1) {
      throw new MorphyEntityIndexException(
          String.format(
              "Height difference at node %d was %d (entity type %s)",
  ...
  /**
   * Upgrades an EntityIndex to the latest version of the file format if needed If the file doesn't
   * exist or is already on the latest version, nothing is done.
   *
   * @param file an entity index file
   */
  protected static void upgrade(@NotNull File file, @NotNull EntityIndexSerializer serializer)
      throws IOException {
    if (!file.exists()) {
      return;
    }
    ...
    try {
      ...
      if (header.headerSize() != 28) {
        throw new MorphyNotSupportedException(
            String.format("Unsupported header size in %s (%d)", file, header.headerSize()));
      }
      ...
      EntityIndexHeader upgradedHeader =
          ImmutableEntityIndexHeader.copyOf(header).withHeaderSize(32);
