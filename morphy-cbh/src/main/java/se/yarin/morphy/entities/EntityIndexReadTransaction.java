...
public class EntityIndexReadTransaction<T extends Entity & Comparable<T>>
  ...
  public @NotNull List<Integer> getDeletedEntityIds() {
    ...
    if (id >= 0) {
      log.warn(
          String.format(
              "Loop in deleted id chain in entity %s", index().entityType().nameSingular()));
    }
    return ids;
  }
   ...
   */
  public @NotNull Iterable<T> iterable(
      @Nullable Integer startId, @Nullable Integer endId, @Nullable EntityFilter<T> filter) {
  ...
  /**
   * Gets an iterable of all entities matching an optional filter in the index sorted by the default
   * sorting order.
   *
  ...
  /**
   * Gets an iterable of all entities in the index starting at a given key (inclusive), ending at a
   * given key (exclusive), sorted by the default sorting order.
   *
  ...
  /**
   * Gets an iterable of all entities matching an optional filter in the index starting at a given
   * key (inclusive), ending at a given key (exclusive), sorted by the default sorting order.
   *
   ...
   */
  public @NotNull Iterable<T> iterableAscending(
      @Nullable T start, @Nullable T end, @Nullable EntityFilter<T> filter) {
    int stopId = -1;
    if (end != null) {
    ...
    return () ->
        new OrderedEntityAscendingIterator<>(
            start == null ? begin() : lowerBound(start), finalStopId, filter);
  }
  ...
  /**
   * Gets an iterable of all entities in the index starting at the given key (inclusive), ending at
   * a given key (exclusive) in reverse default sorting order.
   *
   * @param start the starting key
   * @param end the end key
   * @return an iterable of entities
  ...
  /**
   * Gets an iterable of all entities matching an optional filter in the index starting at the given
   * key (inclusive), ending at a given key (exclusive), sorted in the reverse default sorting
   * order.
   *
   * @param start the starting key
   * @param end the end key
  ...
  public @NotNull Iterable<T> iterableDescending(
    ...
    return () ->
        new OrderedEntityDescendingIterator<>(
            start == null ? end() : upperBound(start), finalStopId, filter);
  }
  ...
  /**
   * Gets a stream of all entities in the index matching an optional filter, sorted by the default
   * sorting order.
   *
   * @param filter an optional filter
   * @return a stream of all entities
  ...
  /**
   * Gets a stream of all entities in the index starting at the given key (inclusive), ending at the
   * given key (exclusive), sorted by the default sorting order.
   *
   * @param start the starting key
   * @param end the end key
  ...
  /**
   * Gets a stream of all entities matching an optional filter in the index starting at a given key
   * (inclusive), and ending at a given key (exclusive), sorted by the default sorting order.
   *
   * @param start the starting key
   * @param end the end key
   * @param filter an optional filter
  ...
  /**
   * Gets a stream of all entities matching an optional filter in the index in reverse default
   * sorting order.
   *
   * @param filter an optional filter
   * @return a stream of all entities
  ...
  /**
   * Gets a stream of all entities in the index starting at the given key (inclusive), ending at the
   * given key (exclusive), in reverse default sorting order.
   *
   * @param start the starting key
   * @param end the end key
   * @return a stream of entities
  ...
  /**
   * Gets a stream of all entities matching an optional filter in the index starting at the given
   * key (inclusive), ending at the given key (exclusive), in reverse default sorting order.
   *
   * @param start the starting key
   * @param end the end key
   * @param filter an optional filter
