...
public abstract class Team extends Entity implements Comparable<Team> {
  ...
  public static Team of(
      @Nullable String title, int teamNumber, boolean season, int year, @Nullable Nation nation) {
    ImmutableTeam.Builder builder = ImmutableTeam.builder();
    if (title != null) {
      builder.title(title);
    }
    ...
    return builder.teamNumber(teamNumber).season(season).year(year).build();
  }
