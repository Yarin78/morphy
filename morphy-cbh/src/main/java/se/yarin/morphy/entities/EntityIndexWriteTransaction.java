...
public class EntityIndexWriteTransaction<T extends Entity & Comparable<T>>
  ...
  /**
   * Looks up an entity based on key and returns the id. If the entity is missing, it's created in
   * the transaction with 0 count in the statistics.
   *
  ...
  public void validateCommit() {
    ...
    if (index().currentVersion() != version()) {
      throw new IllegalStateException(
          String.format(
              "The database has changed since the transaction started (current version = %d, transaction version = %d)",
  ...
  void commit(@Nullable Runnable additionalCommitAction) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            String.format(
                "Committed transaction containing %d node changes", changes.values().size()));
      }
      index().bumpVersion();
      clearChanges();
    } finally {
  ...
  public void putEntityById(int entityId, @NotNull T entity) {
    ...
    if (entityId < 0 || entityId >= header.capacity()) {
      throw new IllegalArgumentException(
          String.format(
              "Can't put an entity with id %d when capacity is %d", entityId, header.capacity()));
    }
    ...
    if (oldEntity.compareTo(entity) == 0) {
      ...
      EntityNode newNode =
          new EntityNode(
              entityId,
              oldNode.getLeftChildId(),
              oldNode.getRightChildId(),
              oldNode.getBalance(),
              entity.count(),
              entity.firstGameId(),
  ...
  /**
   * Updates an entity in the storage. The key fields of the entity will determine which entity in
   * the storage to update.
   *
   * @param entity the new entity. {@link Entity#id()} will be ignored.
   * @throws IllegalArgumentException if there is no matching entity, or if there are multiple
   *     matching entities
  ...
  public int putEntityByKey(@NotNull T entity) {
    ...
    EntityNode newNode =
        new EntityNode(
            treePath.getEntityId(),
            node.getLeftChildId(),
            node.getRightChildId(),
            node.getBalance(),
            entity.count(),
            entity.firstGameId(),
            serializeEntity(entity));
  ...
  public boolean deleteEntity(int entityId) {
    ...
    if (nodePath.isEnd() || nodePath.getEntity().compareTo(entity) != 0) {
      ...
      throw new MorphyEntityIndexException(
          "Broken database structure; couldn't find the node to delete.");
    }
  ...
  private boolean internalDeleteEntity(@NotNull NodePath deleteEntityPath) {
    ...
    if (deleteEntityPath.hasLeftChild() && deleteEntityPath.hasRightChild()) {
      ...
      // TODO: This code is a bit ugly, we cut the successor part so it's rooted at the node we want
      // to delete
      ...
      EntityNode newNode =
          deleteNode.update(
              successorNode.getLeftChildId(),
              successorNode.getRightChildId(),
              successorNode.getBalance());
      int rid = deleteNode.getRightChildId();
      if (rid == successorNode.getId()) {
        rid = deleteNode.getId();
      }
      EntityNode newSuccessorNode =
          successorNode.update(deleteNode.getLeftChildId(), rid, deleteNode.getBalance());
    ...
    int childId =
        deleteNode.getLeftChildId() >= 0
            ? deleteNode.getLeftChildId()
            : deleteNode.getRightChildId();
    replaceChild(parentPath, deleteNodeIsLeftChild, childId);

    // Nothing should now point to the node we want to delete
    EntityNode deletedNode =
        new EntityNode(
            deleteEntityId,
            ENTITY_DELETED,
            header.deletedEntityId(),
            0,
            0,
            0,
            new byte[header.entitySize() - 8]);

    putNode(deletedNode);
    header =
        ImmutableEntityIndexHeader.copyOf(header)
            .withDeletedEntityId(deleteEntityId)
            .withNumEntities(header.numEntities() - 1);

    // Retrace and re-balance tree
    for (boolean nextIsLeftChild = deleteNodeIsLeftChild;
        parentPath != null;
  ...
  // Rotates the tree rooted at x with right child z first to the right then to the left and returns
  // the new root
  ...
  // Rotates the tree rooted at x with left child z first to the left then to the right and returns
  // the new root
