...
public class TournamentYearTitleFilter implements EntityIndexFilter<Tournament> {
  ...
  public TournamentYearTitleFilter(
      int year, @NotNull String title, boolean caseSensitive, boolean exactMatch) {
    this.year = year;
  ...
  private boolean matchesTitle(@NotNull String tournamentTitle) {
    if (exactMatch) {
      return caseSensitive
          ? tournamentTitle.equals(title)
          : tournamentTitle.equalsIgnoreCase(title);
    }
    return caseSensitive
        ? tournamentTitle.startsWith(title)
        : tournamentTitle.toLowerCase().startsWith(title);
  }
