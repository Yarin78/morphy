...
/**
 * Represents the .cbtt file containing additional Tournament information. May contain fewer entries
 * than the .cbt file as entries are only written if they contain any real data, or if needed to
 * fill up the space up until such an entry.
 */
public class TournamentExtraStorage
    implements ItemStorageSerializer<TournamentExtraHeader, TournamentExtra>, MetricsProvider {
  ...
  public TournamentExtraStorage(@Nullable DatabaseContext context) {
    this(
        new InMemoryItemStorage<>(
            context, STORAGE_NAME, TournamentExtraHeader.empty(), TournamentExtra.empty()),
        context);
  }

  public TournamentExtraStorage(
      @NotNull ItemStorage<TournamentExtraHeader, TournamentExtra> storage,
  ...
  protected TournamentExtraStorage(
      @NotNull File file, @NotNull Set<OpenOption> options, @Nullable DatabaseContext context)
      throws IOException {
    this.context = context == null ? new DatabaseContext() : context;
    this.storage =
        new FileItemStorage<>(
            file, this.context, STORAGE_NAME, this, TournamentExtraHeader.empty(), options);
    this.itemMetricsRef = ItemMetrics.register(this.context.instrumentation(), STORAGE_NAME);
    ...
    if (options.contains(WRITE)) {
      if (storage.getHeader().version() < TournamentExtraHeader.DEFAULT_HEADER_VERSION) {
        throw new MorphyNotSupportedException(
            String.format(
                "Old extra tournament storage version; upgrade needed but not yet supported (%d < %d)",
                storage.getHeader().version(), TournamentExtraHeader.DEFAULT_HEADER_VERSION));
      }
      if (storage.getHeader().version() > TournamentExtraHeader.DEFAULT_HEADER_VERSION) {
        throw new MorphyNotSupportedException(
            String.format(
                "Newer unsupported extra tournament storage format; writing not possible (%d > %d)",
                storage.getHeader().version(), TournamentExtraHeader.DEFAULT_HEADER_VERSION));
      }
      if (storage.getHeader().recordSize() != TournamentExtraHeader.DEFAULT_RECORD_SIZE) {
        ...
        throw new MorphyNotSupportedException(
            String.format(
                "Size of extra tournament record was %d but expected %d; writing not possible.",
                storage.getHeader().recordSize(), TournamentExtraHeader.DEFAULT_RECORD_SIZE));
      }
    }
  }

  public static TournamentExtraStorage create(@NotNull File file, @Nullable DatabaseContext context)
      throws IOException {
    return new TournamentExtraStorage(file, Set.of(READ, WRITE, CREATE_NEW), context);
  }

  public static TournamentExtraStorage open(@NotNull File file, @Nullable DatabaseContext context)
      throws IOException {
    return open(file, DatabaseMode.READ_WRITE, context);
  }

  public static TournamentExtraStorage open(
  ...
  @Override
  public @NotNull TournamentExtra deserializeItem(
      int id, @NotNull ByteBuffer buf, @NotNull TournamentExtraHeader header) {
  ...
  public void serializeItem(
      @NotNull TournamentExtra tournamentExtra,
  ...
  public void copyEntities(@NotNull TournamentExtraStorage targetStorage) {
    ...
    ImmutableTournamentExtraHeader newHeader =
        ImmutableTournamentExtraHeader.copyOf(storage.getHeader())
  ...
  /**
   * Upgrades the extra tournament storage to the latest version if necessary If the file doesn't
  ...
  public static void upgrade(@NotNull File file) throws IOException {
    ...
    if (currentHeader.version() < TournamentExtraHeader.DEFAULT_HEADER_VERSION
      ...
      log.info(
          String.format(
              "Upgrading tournament extra storage from version %d (item size %d) to version %d (item size %d)",
              currentHeader.version(),
              currentHeader.recordSize(),
              TournamentExtraHeader.DEFAULT_HEADER_VERSION,
              TournamentExtraHeader.DEFAULT_RECORD_SIZE));

      File upgradedStorageFile = File.createTempFile(CBUtil.baseName(file), ".cbtt");
      upgradedStorageFile.delete();

      TournamentExtraStorage oldStorage =
