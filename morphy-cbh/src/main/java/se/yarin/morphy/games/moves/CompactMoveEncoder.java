...
public class CompactMoveEncoder implements MoveEncoder {
    ...
    private static final CompactMoveEncoder.OpcodeMap[] opcodeMap = new CompactMoveEncoder.OpcodeMap[256];
    ...
    static {
        ...
        for (int op = 1, ofs = 0, rangeNo = 0; op < 256; op++, ofs++) {
            ...
            opcodeMap[op] = ImmutableOpcodeMap.of(opcodeRanges[rangeNo].piece(), ofs, opcodeRanges[rangeNo].pieceNo());
        }
...
//                log.info(String.format("Decoded byte %02X", opcode));
    ...
    private Move decodeTwoByteMove(int opcode, Position board)
        ...
        if (piece == Piece.NO_PIECE) {
            throw new MorphyMoveDecodingException("No piece at source square: " + Chess.sqiToStr(fromSqi));
        }
