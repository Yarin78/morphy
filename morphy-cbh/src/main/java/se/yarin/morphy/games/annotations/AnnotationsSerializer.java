...
public final class AnnotationsSerializer {
  ...
  /**
   * Deserializes a ChessBase annotation from a {@link ByteBuffer}. If an annotation is of an
   * unknown type, {@link UnknownAnnotation} will be returned. If there was some error parsing the
   * annotation data, {@link InvalidAnnotation} will be returned.
   *
   * @param buf the byte buffer
   * @return an annotation
   */
  public static Annotation deserializeAnnotation(@NotNull ByteBuffer buf)
      throws MorphyFatalAnnotationDecodingException {
    ...
    if (annotationSize < 0) {
      throw new MorphyFatalAnnotationDecodingException(
          "Invalid annotation size: " + (annotationSize + 6));
    }
    ...
    try {
      ...
      if (serializer != null) {
        ...
        */
        //                log.warn(String.format("Unknown annotation type %d containing %d bytes of
        // data",                            annotationType, noBytes - 6));
