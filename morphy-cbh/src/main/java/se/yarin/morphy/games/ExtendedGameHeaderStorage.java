...
/**
 * Represents the .cbj file containing additional GameHeader information. This file may be missing
 * in very old databases. There are also multiple versions possible, with different sized records.
 */
public class ExtendedGameHeaderStorage
    implements ItemStorageSerializer<ExtendedGameHeaderStorage.ExtProlog, ExtendedGameHeader>,
        MetricsProvider {
  ...
  private ExtendedGameHeaderStorage(
      @NotNull ItemStorage<ExtProlog, ExtendedGameHeader> storage,
  ...
  private ExtendedGameHeaderStorage(
      @NotNull File file, @NotNull Set<OpenOption> options, @Nullable DatabaseContext context)
      throws IOException {
    if (!CBUtil.extension(file).equals(".cbj")) {
      throw new IllegalArgumentException(
          "The file extension of an extended GameHeader storage must be .cbj");
    }
    this.context = context == null ? new DatabaseContext() : context;
    this.itemMetricsRef = ItemMetrics.register(this.context.instrumentation(), STORAGE_NAME);
    this.storage =
        new FileItemStorage<>(file, this.context, STORAGE_NAME, this, ExtProlog.empty(), options);

    if (options.contains(WRITE)) {
      if (storage.getHeader().version() < ExtProlog.DEFAULT_VERSION) {
        throw new MorphyNotSupportedException(
            "Old extended game header storage version; upgrade needed but not yet supported.");
      }
      if (storage.getHeader().version() > ExtProlog.DEFAULT_VERSION) {
        throw new MorphyNotSupportedException(
            "Newer unsupported extended game header storage format; writing not possible.");
      }
      if (storage.getHeader().serializedItemSize() != ExtProlog.DEFAULT_SERIALIZED_ITEM_SIZE) {
        // This shouldn't happen because of the version checks above, so this is mostly a sanity
        // check
        // to ensure that same version doesn't have different record sizes.
        throw new MorphyNotSupportedException(
            String.format(
                "Size of extended game header record was %d but expected %d; writing not possible.",
                storage.getHeader().serializedItemSize(), ExtProlog.DEFAULT_SERIALIZED_ITEM_SIZE));
      }
    }
  }

  public static ExtendedGameHeaderStorage create(
  ...
  public static ExtendedGameHeaderStorage open(
      @NotNull File file, @Nullable DatabaseContext context) throws IOException {
  ...
  public long numDiskPages() {
    if (storage instanceof FileItemStorage) {
      return ((FileItemStorage<ExtendedGameHeaderStorage.ExtProlog, ExtendedGameHeader>) storage)
          .numPages();
    } else {
      return 0;
    }
  ...
  public static void upgrade(@NotNull File cbhFile) throws IOException {
    ...
    if (!cbhFile.exists()) {
      // No need to upgrade if the main game header index doesn't even exist (e.g. when creating a
      // new database)
      return;
    }
    ...
    if (!cbjFile.exists()) {
      ...
      if (currentProlog.version() < ExtProlog.DEFAULT_VERSION
          || currentProlog.serializedItemSize() < ExtProlog.DEFAULT_SERIALIZED_ITEM_SIZE) {
        log.info(
            String.format(
                "Upgrading extended header storage from version %d (item size %d) to version %d (item size %d)",
                currentProlog.version(),
                currentProlog.serializedItemSize(),
                ExtProlog.DEFAULT_VERSION,
                ExtProlog.DEFAULT_SERIALIZED_ITEM_SIZE));

        ExtendedGameHeaderStorage extendedStorage =
            ExtendedGameHeaderStorage.open(cbjFile, DatabaseMode.READ_ONLY, null);
        File upgradedStorageFile = File.createTempFile(CBUtil.baseName(cbhFile), ".cbj");
        upgradedStorageFile.delete();
        ExtendedGameHeaderStorage upgradedStorage =
            ExtendedGameHeaderStorage.create(upgradedStorageFile, null);
      ...
      if (currentProlog.version() < ExtProlog.DEFAULT_VERSION
          || currentProlog.serializedItemSize() < ExtProlog.DEFAULT_SERIALIZED_ITEM_SIZE) {
        throw new MorphyException(
            "Upgrade of extended storage failed!? Still at the wrong version");
      }

      if (currentProlog.numHeaders() < index.count()) {
        ExtendedGameHeaderStorage extendedStorage =
            ExtendedGameHeaderStorage.open(cbjFile, DatabaseMode.READ_WRITE, null);
  ...
  /**
   * Gets a list of all extended game headers between startId (inclusive) and endId (exclusive). The
   * filter is applied on the serialized data, making this operation fast.
   *
   * @param filter the filter, or null to match all extended game headers
   * @return a list of extended game headers. If a header doesn't match the filter, that position in
   *     the list will have a null value. This is to allow easy zip join with {@link
   *     GameHeaderIndex#getFiltered(ItemStorageFilter)}
   */
  public @NotNull List<ExtendedGameHeader> getFiltered(
   ...
   * @param endId the id of the last game header (exclusive)
   * @param filter the filter, or null to match all extended game headers
   * @return a list of extended game headers. If a filter is specified and a header doesn't match,
   *     that position in the list will have a null value.
   */
  public @NotNull List<ExtendedGameHeader> getRange(
      int startId, int endId, @Nullable ItemStorageFilter<ExtendedGameHeader> filter) {
    if (endId < startId) {
      throw new IllegalArgumentException(
          String.format("endId can't be less than startId (%d < %d)", endId, startId));
    }
  ...
  public void copyGameHeaders(ExtendedGameHeaderStorage target) {
    if (target.count() != 0) {
      // Since this is a low-level copy, things will not work if there already are games in the
      // target storage
  ...
  @Value.Immutable
  public abstract static class ExtProlog {
  ...
  @Override
  public @NotNull ExtProlog deserializeHeader(@NotNull ByteBuffer buf)
  ...
  @Override
  public @NotNull ExtendedGameHeader deserializeItem(
      int id, @NotNull ByteBuffer buf, @NotNull ExtProlog header) {
  ...
  public void serializeItem(
      @NotNull ExtendedGameHeader item, @NotNull ByteBuffer buf, @NotNull ExtProlog header) {
    ...
    for (int i = ExtProlog.DEFAULT_SERIALIZED_ITEM_SIZE;
        i < this.storage.getHeader().serializedItemSize();
        i++) {
      buf.put((byte) 0);
    }
  }
