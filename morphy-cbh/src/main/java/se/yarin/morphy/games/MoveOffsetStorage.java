...
public class MoveOffsetStorage {
    ...
    private MoveOffsetStorage(
        ...
        if (!CBUtil.extension(file).equals(".cbgi")) {
            throw new IllegalArgumentException("The file extension of a MoveOffsetStorage index must be .cbgi");
        }
    ...
    public static MoveOffsetStorage open(@NotNull File file, @Nullable DatabaseContext context) throws IOException {
        return open(file, DatabaseMode.READ_WRITE, context);
    }

    public static MoveOffsetStorage open(@NotNull File file, @NotNull DatabaseMode mode, @Nullable DatabaseContext context) throws IOException {
    ...
    public int getOffset(int gameId) {
        ...
        if (gameId < 1 || gameId > numGames) {
            if (strict) {
                throw new IllegalArgumentException("gameId must be between 1 and number of games in the storage");
            }
            return 0;
        }
