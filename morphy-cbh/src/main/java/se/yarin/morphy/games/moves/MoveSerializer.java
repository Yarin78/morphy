...
public final class MoveSerializer {
  ...
  public ByteBuffer serializeMoves(@NotNull GameMovesModel model) {
    // TODO: All serialize method should probably not return a ByteBuffer but write to an existing
    // one
  ...
  /**
   * Deserializes the moves of a ChessBase encoded chess game. If there was some error decoding the
   * game
   *
   * @param buf a buffer containing the serialized game
   * @return a model of the game
   * @throws MorphyMoveDecodingException if there was an error deserializing the moves
   */
  public GameMovesModel deserializeMoves(ByteBuffer buf) throws MorphyMoveDecodingException {
   ...
   * @return a model of the game
   * @throws MorphyMoveDecodingException if there was an error deserializing the moves, with the
   *     {@link MorphyMoveDecodingException#getModel()} containing the moves parsed so far.
   */
  public GameMovesModel deserializeMoves(ByteBuffer buf, boolean checkLegalMoves, int gameId)
    ...
    try {
      ...
      throw new MorphyMoveDecodingException("Moves data header ended abruptly", e);
    }
    ...
    if (encodingMode != 0 && encodingMode != 10) {
      log.warn(
          String.format(
              "Move data in game " + gameId + " has an unusual encoding: %02X", encodingMode));
    }

    if (log.isDebugEnabled()) {
      log.debug(
          String.format(
              "Parsing move data for game " + gameId + " at moveData pos %s with %d bytes left",
              model.root().position().toString("|"),
              moveBuf.limit() - moveBuf.position()));
    }
    ...
    return switch (encodingMode) {
      ...
      default ->
          throw new UnsupportedOperationException(
              String.format("Unsupported move encoder: %02X", encodingMode));
    };
  }
  ...
  public GameMovesModel parseInitialPosition(ByteBuffer buf, boolean hasExtraInfo, int gameId)
    ...
    if (b != 1) {
      // Version?
      log.warn(
          String.format("Unexpected first byte in setup position in game " + gameId + ": %02X", b));
    }
            ...
            switch (pieceNo) {
              ...
              default ->
                  throw new MorphyMoveDecodingException(
                      "Invalid piece in setup position in game " + gameId + ": " + pieceNo);
            };
    ...
    int sp =
        hasExtraInfo
            ? deserializeChess960StartingPosition(buf, stones, gameId)
            : Chess960.REGULAR_CHESS_SP;

    if (sp < 0 || sp >= 960) {
      throw new MorphyMoveDecodingException(
          "Invalid Chess960 position " + sp + " in game " + gameId);
    }
  ...
  private int deserializeChess960StartingPosition(ByteBuffer buf, Stone[] stones, int gameId) {
    ...
    if (spNo < 0 || spNo >= 960) {
      ...
      // This happens on some games in Mega Database 2016 and is probably due to a bug in an earlier
      // Chessbase version
      log.warn(
          "The specific positions of the kings and rooks doesn't match the Chess960 start position number in game "
              + gameId);
    }

    // The encoded start position is wrong, so we need to figure out what it should have been.
    // We really only need the information from the 6 first bytes, but our position model doesn't
    // use them.
    ...
    try {
      ...
      if (chess960Matches(sp, wkSqi, bkSqi, wkrSqi, wqrSqi, bkrSqi, bqrSqi)) {
        log.debug(
            "Deduced the Chess960 starting position number in game " + gameId + " to be " + spNo);
        return spNo;
      }
  ...
  private boolean chess960Matches(
      Position sp, int wkSqi, int bkSqi, int wkrSqi, int wqrSqi, int bkrSqi, int bqrSqi) {
    return sp.stoneAt(wkSqi) == Stone.WHITE_KING
        && sp.stoneAt(bkSqi) == Stone.BLACK_KING
        && sp.stoneAt(wkrSqi) == Stone.WHITE_ROOK
        && sp.stoneAt(wqrSqi) == Stone.WHITE_ROOK
        && sp.stoneAt(bkrSqi) == Stone.BLACK_ROOK
        && sp.stoneAt(bqrSqi) == Stone.BLACK_ROOK;
  }

  public ByteBuffer serializeInitialPosition(
      GameMovesModel moves, ByteBuffer buf, boolean addExtraInfo) {
