...
public final class MoveSerializer {
    ...
    public GameMovesModel parseInitialPosition(ByteBuffer buf, boolean hasExtraInfo, int gameId)
        ...
        for (int i = 0; i < 64; i++) {
            if (byteBufferBitReader.getBit() == 0) {
                ...
                piece = switch (pieceNo) {
                    ...
                    case 5 -> Piece.ROOK;
                    case 6 -> Piece.PAWN;
                    default -> throw new MorphyMoveDecodingException("Invalid piece in setup position in game " + gameId + ": " + pieceNo);
                };
        ...
        if (sp < 0 || sp >= 960) {
            throw new MorphyMoveDecodingException("Invalid Chess960 position " + sp + " in game " + gameId);
        }
