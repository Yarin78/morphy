...
public class TeamFilter
    implements ItemStorageFilter<ExtendedGameHeader>, GameFilter, GameEntityFilter<Team> {
  ...
  public TeamFilter(int[] teamIds, @Nullable GameEntityJoinCondition matchCondition) {
    ...
    // This ought to be supported, but requires that we have the default GameHeader as well as
    // ExtendedGameHeader
    if (matchCondition == GameEntityJoinCondition.WINNER
  ...
  public TeamFilter(
      @NotNull Collection<Team> teams, @Nullable GameEntityJoinCondition matchCondition) {
  ...
  public boolean matches(int id, @NotNull ExtendedGameHeader extendedGameHeader) {
    return matchCondition.matches(
        extendedGameHeader.whiteTeamId(),
        extendedGameHeader.blackTeamId(),
        GameResult.DRAW,
        teamIds);
  }
  ...
  public String toString() {
    ...
    if (teamIds.size() == 1) {
      ...
      s =
          "teamId in ( "
              + teamIds.stream().map(Object::toString).collect(Collectors.joining(", "))
              + ")";
    }
