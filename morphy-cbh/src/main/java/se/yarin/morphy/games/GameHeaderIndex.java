...
public class GameHeaderIndex
  ...
  private GameHeaderIndex(
    ...
    this.storage =
        new FileItemStorage<>(file, this.context, "GameHeader", this, Prolog.empty(), options);
    if (strict) {
      if (storage.getHeader().serializedItemSize() != Prolog.DEFAULT_SERIALIZED_ITEM_SIZE) {
        throw new MorphyNotSupportedException(
            "Game header item size mismatches; writes not possible.");
      }
    }
  }
  ...
  public static GameHeaderIndex open(@NotNull File file, @Nullable DatabaseContext context)
      throws IOException {
    return open(file, DatabaseMode.READ_WRITE, context);
  }

  public static GameHeaderIndex open(
  ...
  /**
   * Gets a game header from the index If the id is invalid (larger than the capacity of the index),
  ...
  public int add(@NotNull GameHeader gameHeader) {
    ...
    ImmutableProlog newProlog =
        ImmutableProlog.copyOf(storage.getHeader())
  ...
  /**
   * Gets a list of all game headers matching a filter in the index. The filter is applied on the
   * serialized data, making this operation fast.
   *
   * @param filter the filter, or null to match all game headers
   * @return a list of game headers. If a header doesn't match the filter, that position in the list
   *     will have a null value. This is to allow easy zip join with {@link
   *     ExtendedGameHeaderStorage#getFiltered(ItemStorageFilter)}
   */
  ...
  /**
   * Gets a list of all game headers that matches a filter between startId (inclusive) and endId
   * (exclusive)
   *
   * @param startId the id of first game header (inclusive)
   * @param endId the id of the last game header (exclusive)
   * @param filter the filter, or null to match all game headers
   * @return a list of game headers. If a filter is specified and a header doesn't match, that
   *     position in the list will have a null value.
   */
  public @NotNull List<GameHeader> getRange(
      int startId, int endId, @Nullable ItemStorageFilter<GameHeader> filter) {
    if (endId < startId) {
  ...
  /** Represents the Prolog in the CBH file. See games.md for more information */
  @Value.Immutable
  public abstract static class Prolog {
    ...
    @Value.Default
    public int nextEmbeddedSoundId() {
      return 0;
    }

    @Value.Default
    public int nextEmbeddedPictureId() {
      return 0;
    }

    @Value.Default
    public int nextEmbeddedVideoId() {
      return 0;
    }
  ...
  @Override
  public @NotNull Prolog deserializeHeader(@NotNull ByteBuffer buf)
      throws MorphyInvalidDataException {
    ImmutableProlog prolog =
        ImmutableProlog.builder()
    ...
    if (prolog.unknownByte1() != 0
        ...
        || prolog.unknownShort1() != 0
        || prolog.unknownShort2() != 0
        || prolog.unknownInt1() != 0
        || prolog.unknownInt2() != 0
        || prolog.unknownInt3() != 0
        || prolog.unknownInt4() != 0) {
      log.warn(
          String.format(
              "Unknown prolog bytes did not have the default values: %d %d %d %d %d %d %d %d",
              prolog.unknownByte1(),
              prolog.unknownByte2(),
              prolog.unknownShort1(),
              prolog.unknownShort2(),
              prolog.unknownInt1(),
              prolog.unknownInt2(),
              prolog.unknownInt3(),
              prolog.unknownInt4()));
    ...
    if (prolog.nextGameId2() != prolog.nextGameId() && prolog.nextGameId2() != 0) {
      log.warn(
          String.format(
              "Second nextGameId didn't match the first one (%d != %d)",
              prolog.nextGameId(), prolog.nextGameId2()));
    }
  ...
  @Override
  public @NotNull GameHeader deserializeItem(
      int gameHeaderId, @NotNull ByteBuffer buf, @NotNull Prolog header) {
    ...
    if (guidingText) {
      ...
      if ((flagInt & ~GameHeaderFlags.allFlagsMask()) > 0) {
        log.warn(
            String.format("Unknown game header flags in game %d: %08X", gameHeaderId, flagInt));
      }
      ...
      if ((annotationMagnitudeFlags & ~(3 | 4 | 8 | 16 | 32 | 128 | 512)) != 0) {
        log.warn(
            "Unknown annotationMagnitudeFlags value "
                + annotationMagnitudeFlags
                + " in game "
                + gameHeaderId);
      }
    ...
    if (buf.position() - oldPosition != Prolog.DEFAULT_SERIALIZED_ITEM_SIZE) {
      throw new MorphyInternalException(
          String.format(
              "Internal error deserializing GameHeader with id %d; %d bytes deserialized, expected %d bytes",
  ...
  public void serializeItem(
      @NotNull GameHeader gameHeader, @NotNull ByteBuffer buf, @NotNull Prolog header) {
