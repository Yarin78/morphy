...
public class AnnotatorFilter
    implements ItemStorageFilter<GameHeader>, GameFilter, GameEntityFilter<Annotator> {
  ...
  public String toString() {
    if (annotatorIds.size() == 1) {
      ...
      return "annotatorId in ( "
          + annotatorIds.stream().map(Object::toString).collect(Collectors.joining(", "))
          + ")";
    }
  }
}
