...
public class MoveRepository implements BlobSizeRetriever {
  ...
  private int overrideEncodingMode =
      -1; // The encoding mode to use when writing games, -1 = default based on type

  private boolean validateDecodedMoves = true; // If true, validate all moves when decoding

  public @NotNull BlobStorage getStorage() {
    return storage;
  }

  public @NotNull MoveSerializer moveSerializer() {
  ...
  public void setValidateDecodedMoves(boolean validateDecodedMoves) {
    this.validateDecodedMoves = validateDecodedMoves;
  }
  ...
  private MoveRepository(
      @NotNull File file, @NotNull Set<OpenOption> openOptions, @Nullable DatabaseContext context)
      throws IOException {
    this.context = context == null ? new DatabaseContext() : context;
  ...
  public static MoveRepository create(@NotNull File file, @Nullable DatabaseContext context)
      throws IOException {
   ...
   */
  public static MoveRepository open(@NotNull File file, @Nullable DatabaseContext context)
      throws IOException {
   ...
   * @throws IOException if something went wrong when opening the repository
   */
  public static MoveRepository open(
  ...
  /**
   * Loads a move repository from file into an in-memory repository. Any writes to the repository
   ...
   */
  protected static BlobStorage loadInMemoryStorage(
  ...
  /**
   * Gets the serialized bytes that make up the moves of the game or the text for guiding texts.
   *
   ...
   * @throws se.yarin.morphy.exceptions.MorphyIOException if there was some IO errors when reading
   *     the moves
   */
   ...
   *
   * @param ofs the old offset where moves of this game was stored, or 0 if this is a new game in
   *     the repository
   * @param model the game to store
   * @return The offset where the game moves was stored
   * @throws se.yarin.morphy.exceptions.MorphyIOException if there was some IO errors when storing
   *     the moves
   */
   ...
   * Puts the moves of a game into the move repository
   *
   * @param offset the old offset where moves of this game was stored, or 0 if this is a new game in
   *     the repository
