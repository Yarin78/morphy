...
public class PlayerFilter extends IsGameFilter implements GameEntityFilter<Player> {
  ...
  public PlayerFilter(
      @NotNull Collection<Player> players, @Nullable GameEntityJoinCondition matchCondition) {
    this.playerIds =
  ...
  public boolean matches(int id, @NotNull GameHeader gameHeader) {
    ...
    return matchCondition.matches(
        gameHeader.whitePlayerId(), gameHeader.blackPlayerId(), gameHeader.result(), playerIds);
  }
  ...
  public String toString() {
    ...
    if (playerIds.size() == 1) {
      ...
      s =
          "playerId in ( "
              + playerIds.stream().map(Object::toString).collect(Collectors.joining(", "))
              + ")";
    }
