...
public abstract class DatabaseTransaction extends TransactionBase implements EntityRetriever {
  ...
  public @NotNull MetricsRepository metrics() {
    return metrics;
  }
  ...
  public @NotNull Game getGame(int gameId) {
    ...
    // We've already verified when opening the database that it's okay to fill out with empty
    // extended headers
    // if they are missing entirely or just fewer
    ExtendedGameHeader extendedGameHeader =
        gameId <= storage.count() ? storage.get(gameId) : ExtendedGameHeader.empty(gameHeader);
