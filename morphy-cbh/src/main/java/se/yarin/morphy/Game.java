...
public class Game implements IdObject {
  ...
  public Game(
      @NotNull DatabaseWriteTransaction transaction,
      @NotNull GameHeader header,
      @NotNull ExtendedGameHeader extendedHeader) {
  ...
  /**
   * @return the transaction the game is part of, or null if the game is already committed to the
   *     database
   */
  ...
  public int gameTagId() {
    return extendedHeader.gameTagId();
  }
  ...
  public long getAnnotationOffset() {
    return resolveOffset(
        header.annotationOffset(), extendedHeader.annotationOffset(), "annotation");
  }
  ...
  private long resolveOffset(int shortOffset, long longOffset, @NotNull String type) {
    ...
    if (longOffset != 0 && log.isDebugEnabled()) {
      ...
      log.debug(
          String.format(
              "The %s offset differs between the two header databases (%d != %d)",
              type, shortOffset, longOffset));
    }
