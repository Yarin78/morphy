...
public class DatabaseCbh implements EntityRetriever, Database {
  ...
  public static final List<String> MANDATORY_EXTENSIONS =
      List.of(".cbh", ".cbg", ".cba", ".cbp", ".cbt", ".cbc", ".cbs");
  public static final List<String> ADDITIONAL_EXTENSIONS =
      List.of(".cbtt", ".cbj", ".cbe", ".cbl", ".cbm", ".flags");
  public static final List<String> SEARCH_BOOSTER_EXTENSIONS =
      List.of(".cbb", ".cbgi", ".cit", ".cib", ".cit2", ".cib2");

  public static final List<String> ALL_EXTENSIONS =
      Stream.concat(
              Stream.concat(
                  DatabaseCbh.MANDATORY_EXTENSIONS.stream(), DatabaseCbh.ADDITIONAL_EXTENSIONS.stream()),
              DatabaseCbh.SEARCH_BOOSTER_EXTENSIONS.stream())
  ...
  @NotNull
  public GameHeaderIndex gameHeaderIndex() {
    return gameHeaderIndex;
  }

  @NotNull
  public ExtendedGameHeaderStorage extendedGameHeaderStorage() {
  ...
  @NotNull
  public TournamentIndex tournamentIndex() {
    return tournamentIndex;
  }

  @NotNull
  public TournamentExtraStorage tournamentExtraStorage() {
    return tournamentExtraStorage;
  }

  @NotNull
  public AnnotatorIndex annotatorIndex() {
  ...
  @NotNull
  public GameTagIndex gameTagIndex() {
    return gameTagIndex;
  }

  @NotNull
  public TopGamesStorage topGamesStorage() {
    return topGamesStorage;
  }

  @Nullable
  public GameEntityIndex gameEntityIndex(@NotNull EntityType type) {
    return type != EntityType.GAME_TAG ? gameEntityIndexPrimary : gameEntityIndexSecondary;
  }

  @Nullable
  public GameEntityIndex gameEntityIndexPrimary() {
    return gameEntityIndexPrimary;
  }

  @Nullable
  public GameEntityIndex gameEntityIndexSecondary() {
    return gameEntityIndexSecondary;
  }

  @Nullable
  public MoveOffsetStorage moveOffsetStorage() {
    return moveOffsetStorage;
  }

  @Nullable
  public GameEventStorage gameEventStorage() {
   ...
   *
   * <p>Important: Write operations to the database will not be persisted to disk.
   */
  public DatabaseCbh() {
    this(null);
  }
  ...
  public DatabaseCbh(@Nullable DatabaseConfig config) {
    ...
    this.gameEntityIndexSecondary =
        new GameEntityIndex(GameEntityIndex.SECONDARY_TYPES, this.context);
  ...
  private DatabaseCbh(
    ...
    Set<DatabaseContext> contexts =
        new HashSet<>(
            Arrays.asList(
                context,
                gameHeaderIndex.context(),
                extendedGameHeaderStorage.context(),
                moveRepository.context(),
                annotationRepository.context(),
                playerIndex.context(),
                tournamentIndex.context(),
                tournamentExtraStorage.context(),
                annotatorIndex.context(),
                sourceIndex.context(),
                teamIndex.context(),
                gameTagIndex.context(),
                topGamesStorage.context()));
  ...
  public static DatabaseCbh create(@NotNull File file) throws IOException {
    return create(file, false);
  }

  public static DatabaseCbh create(@NotNull File file, boolean overwrite) throws IOException {
    ...
    MoveRepository moveRepository =
        MoveRepository.create(CBUtil.fileWithExtension(file, ".cbg"), context);
    AnnotationRepository annotationRepository =
        AnnotationRepository.create(CBUtil.fileWithExtension(file, ".cba"), context);
    PlayerIndex playerIndex = PlayerIndex.create(CBUtil.fileWithExtension(file, ".cbp"), context);
    TournamentIndex tournamentIndex =
        TournamentIndex.create(CBUtil.fileWithExtension(file, ".cbt"), context);
    AnnotatorIndex annotatorIndex =
        AnnotatorIndex.create(CBUtil.fileWithExtension(file, ".cbc"), context);
    SourceIndex sourceIndex = SourceIndex.create(CBUtil.fileWithExtension(file, ".cbs"), context);

    ExtendedGameHeaderStorage extendedGameHeaderStorage =
        ExtendedGameHeaderStorage.create(CBUtil.fileWithExtension(file, ".cbj"), context);
    TournamentExtraStorage tournamentExtraStorage =
        TournamentExtraStorage.create(CBUtil.fileWithExtension(file, ".cbtt"), context);
    TeamIndex teamIndex = TeamIndex.create(CBUtil.fileWithExtension(file, ".cbe"), context);
    GameTagIndex gameTagIndex =
        GameTagIndex.create(CBUtil.fileWithExtension(file, ".cbl"), context);

    TopGamesStorage topGamesStorage =
        TopGamesStorage.create(CBUtil.fileWithExtension(file, ".flags"), context);

    GameEntityIndex gameEntityIndex =
        GameEntityIndex.create(
    ...
    MoveOffsetStorage moveOffsetStorage =
        MoveOffsetStorage.create(CBUtil.fileWithExtension(file, ".cbgi"), context);
    GameEventStorage gameEventStorage =
        GameEventStorage.create(CBUtil.fileWithExtension(file, ".cbb"), context);

    return new DatabaseCbh(
        file.getName(),
        context,
        gameHeaderIndex,
   ...
   *
   * <p>Make sure not to open the same database multiple times, as that could lead to an
   * inconsistent database! It's recommended that the caller keeps a lock on the database file
   * itself.
   *
   ...
   * @throws IOException if an IO error occurred when opening the database; this may happen if some
   *     mandatory files are missing
   */
  public static DatabaseCbh open(@NotNull File file, @NotNull DatabaseMode mode) throws IOException {
    if (!CBUtil.extension(file).equals(".cbh")) {
      throw new IllegalArgumentException("The extension of the database file must be .cbh");
    }
    ...
    if (mode == DatabaseMode.READ_WRITE) {
      ...
      // their header being shorter than they would be in a new database (10 bytes instead of 26
      // bytes);
    ...
    MoveRepository moveRepository =
        MoveRepository.open(CBUtil.fileWithExtension(file, ".cbg"), mode, context);
    AnnotationRepository annotationRepository =
        AnnotationRepository.open(CBUtil.fileWithExtension(file, ".cba"), mode, context);
    PlayerIndex playerIndex =
        PlayerIndex.open(CBUtil.fileWithExtension(file, ".cbp"), mode, context);
    TournamentIndex tournamentIndex =
        TournamentIndex.open(CBUtil.fileWithExtension(file, ".cbt"), mode, context);
    AnnotatorIndex annotatorIndex =
        AnnotatorIndex.open(CBUtil.fileWithExtension(file, ".cbc"), mode, context);
    SourceIndex sourceIndex =
        SourceIndex.open(CBUtil.fileWithExtension(file, ".cbs"), mode, context);

    // Optional files. Only in very early ChessBase databases are they missing.
    // If the database is opened in read-only mode, don't create them but instead provide empty
    // versions
    ...
    TeamIndex teamIndex =
        cbeFile.exists() ? TeamIndex.open(cbeFile, mode, context) : new TeamIndex(context);
    GameTagIndex gameTagIndex =
        cblFile.exists() ? GameTagIndex.open(cblFile, mode, context) : new GameTagIndex(context);
    TopGamesStorage topGamesStorage =
        flagsFile.exists()
            ? TopGamesStorage.open(flagsFile, mode, context)
            : new TopGamesStorage(context);
    GameEventStorage gameEventStorage =
        gameEventsFile.exists()
            ? GameEventStorage.open(gameEventsFile, mode, context)
            : new GameEventStorage(context);
    ...
    if (mismatch) {
      throw new MorphyInvalidDataException(
          String.format(
              "The number of headers mismatch in the cbh and cbj files mismatch (%d != %d)",
   ...
   * @throws IOException if there are some files in the same folder as file with the same name (case
   *     insensitive, excluding extension)
   */
  private static void ensureNoDatabaseExists(@NotNull File file) throws IOException {
    ...
    if (files.length > 0) {
      throw new IOException(
          String.format(
              "Can't create a database %s because file %s exists",
              file.getPath(), files[0].getPath()));
    }
  }

  /**
   * Deletes a database from disk by deleting all database related files and subdirectories with the
   * same basename as the given file.
   *
  ...
  public static void delete(@NotNull File file) throws IOException {
    ...
    // Collect all files that should be deleted and verify that the process has access to delete
    // them all
  ...
  /**
   * Gets all games in the database If the database is large, use the iterable options in
   * DatabaseReadTransaction instead.
   *
  ...
  /**
   * Gets all games in the database matching a filter by doing a sequence scan. Consider using the
   * GameSearcher instead for more performant game searches.
   *
