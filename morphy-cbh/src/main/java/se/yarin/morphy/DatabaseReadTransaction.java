...
public class DatabaseReadTransaction extends DatabaseTransaction {
  ...
  public DatabaseReadTransaction(@NotNull Database database) {
    ...
    this.tournamentTransaction =
        database.tournamentIndex().beginReadTransaction(database.tournamentExtraStorage());
  ...
  public <T extends Entity & Comparable<T>> EntityIndexReadTransaction<?> entityTransaction(
  ...
  /**
   * Returns an iterable of all games between startId and endId matching the given filter in the
   * index, sorted by id.
   *
   * @param startId the first id in the iterable (inclusive)
   * @param endId the last id in the iterable (exclusive)
   ...
   */
  public @NotNull Iterable<Game> iterable(
      @Nullable Integer startId, @Nullable Integer endId, @Nullable GameFilter filter) {
