...
public abstract class KeyProvider {
  ...
  static {
    ...
    for (int i = 0; i < GROUP_COUNT; i++) {
      InputStream stream =
          KeyProvider.class.getResourceAsStream(String.format("keys/key_%d.bin", i));
      try {
    ...
    for (int group = 0; group < cb13Offsets.length; group++) {
      FileOutputStream fos =
          new FileOutputStream(
              String.format("src/main/resources/se/yarin/morphy/util/keys/key_%d.bin", group));
  ...
  /**
   * These are the offsets into CBase13.exe where the keys are fetched from. They were found by
   * searching for sequences of 256 bytes that contained a permutation of the list [0..256)
   *
   * <p>They are divided into groups where there are gaps in the offsets. Most groups have 32 keys.
   *
   * <p>Most (but not all) keys come in pairs - first an encryption key, then the corresponding
   * decryption key.
   */
  public static int[][] cb13Offsets =
      new int[][] {
        new int[] {
