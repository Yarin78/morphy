...
public class DatabaseWriteTransactionTest extends DatabaseTestSetup {
  ...
  public void addGameCausingNewEntitiesOfAllTypesToBeAdded() {
    try (var txn = new DatabaseWriteTransaction(testBase)) {
      putTestGame(txn, 0, "foo - bar", "foo", "foo", "foo", "foo - bar", "foo", 100, 100, 0, 0);
      txn.commit();
    }
  ...
  public void addGameWithNewSource() {
    // Check that if a new source is created when a game is added, all the metadata fields are also
    // set
    GameModel gameModel = new GameModel();
  ...
  public void addGameWithNewTournamentIncludingExtraFields() {
    // Check that if a new event is created when a game is added, all the metadata fields are also
    // set
  ...
  public void addGameWithExistingTournamentHavingMoreDetails() {
    ...
    assertEquals(
        new Date(2021, 4, 15), testBase.tournamentExtraStorage().get(tour2.id()).endDate());
  }
  ...
  public void setEntityAfterGameUpdate() {
    ...
    try (var txn = new DatabaseWriteTransaction(testBase)) {
      ...
      txn.updateGameTagById(
          game.gameTagId(),
          ImmutableGameTag.builder().englishTitle("eng").germanTitle("ger").build());
      txn.commit();
    }
  ...
  public void multipleAddsAndReplacesInSameBatchInMixedOrder() {
    List<Consumer<DatabaseWriteTransaction>> operations =
        Arrays.asList(
            ...
            (DatabaseWriteTransaction txn) ->
                putTestGame(txn, 11, "So - Mamedyarov", 130, 500, 16, 11),
            ...
            (DatabaseWriteTransaction txn) ->
                putTestGame(txn, 12, "Carlsen - Grischuk", 120, 200, 18, 18),
            // #18 Add new game with new player
            (DatabaseWriteTransaction txn) ->
                putTestGame(txn, 0, "Kramnik - Kasparov", 200, 2000, 18, 18),
            // Update last game so new player no longer exists
            (DatabaseWriteTransaction txn) ->
                putTestGame(txn, 18, "Kasparov - Carlsen", 200, 0, 18, 18),
            // Rename player
            (DatabaseWriteTransaction txn) ->
                txn.updatePlayerById(
                    txn.getGame(18).whitePlayerId(), Player.ofFullName("Kasparov, Garry")),
            // #19 Add with newly renamed entity
            (DatabaseWriteTransaction txn) ->
                putTestGame(txn, 0, "Caruana - Kasparov, Garry", 30, 100, 19, 19),
            // Update annotations to old game
            (DatabaseWriteTransaction txn) ->
                putTestGame(txn, 9, "Radjabov - Nepo", 900, 20000, 18, 18));

    // The database resulting resulting from committing after each operation will be the expected
    // result
    ...
    // Try split up the operations into three batches, to make sure all subsets combinations are
    // tried in a batch
  ...
  public void addTextToDatabase() {
    ...
    assertEquals(
        ...
        testBase
            .tournamentIndex()
            .get(Tournament.of("tour-with-text", new Date(2021, 5, 1)))
            .count());
