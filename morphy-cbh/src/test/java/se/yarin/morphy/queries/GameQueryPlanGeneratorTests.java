...
public class GameQueryPlanGeneratorTests {
  ...
  public void gamesBySingleFixedPlayer() {
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      ...
      this.assertPlanExists(
          ...
          new GameLookup(
              qc,
              new GameIdsByEntities<>(qc, new Manual<>(qc, Set.of(7)), EntityType.PLAYER),
              null));
    }
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      ...
      this.assertPlanExists(
          plans, new GameIdsByEntities<>(qc, new Manual<>(qc, Set.of(7)), EntityType.PLAYER));
    }
  }
              ...
              new Distinct<>(
                  ...
                  new Sort<>(
                      ...
                      new GameIdsByEntities<>(
                          qc, new Manual<>(qc, Set.of(7, 15, 19)), EntityType.PLAYER),
                      QuerySortOrder.byId())),
              null));
    }
  }
  ...
  public void gamesByFixedPlayerAndRegularGameFilter() {
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      ...
      this.assertPlanExists(
          ...
          new MergeJoin<>(
              qc,
              new GameTableScan(qc, combinedFilter),
              new GameIdsByEntities<>(qc, new Manual<>(qc, Set.of(7)), EntityType.PLAYER)));
    }
  }
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      ...
      this.assertPlanExists(
          ...
          new GameLookup(
              qc,
              new GameIdsByEntities<>(qc, new Manual<>(qc, Set.of(7)), EntityType.PLAYER),
              CombinedGameFilter.combine(List.of(playerFilter, tournamentFilter, sourceFilter))));
      this.assertPlanExists(
          plans,
          new GameLookup(
              qc,
              new GameIdsByEntities<>(qc, new Manual<>(qc, Set.of(8)), EntityType.TOURNAMENT),
              CombinedGameFilter.combine(List.of(playerFilter, sourceFilter))));
      this.assertPlanExists(
          plans,
          new GameLookup(
              qc,
              new GameIdsByEntities<>(qc, new Manual<>(qc, Set.of(9)), EntityType.SOURCE),
              CombinedGameFilter.combine(List.of(playerFilter, tournamentFilter))));
      ...
      this.assertPlanExists(
          ...
          new GameLookup(
              ...
              new MergeJoin<>(
                  ...
                  new GameIdsByEntities<>(
                      qc, new Manual<Tournament>(qc, Set.of(8)), EntityType.TOURNAMENT)),
              CombinedGameFilter.combine(List.of(playerFilter, sourceFilter))));
    }
  }
  ...
  public void gamesBySimplePlayerJoin() {
    ...
    GameEntityJoin<Player> join =
        new GameEntityJoin<>(
            new EntityQuery<Player>(db, EntityType.PLAYER, List.of(kasparovFilter)),
      ...
      this.assertPlanExists(
          ...
          new GameEntityLoopJoin<>(
              ...
              new GameLookup(
                  ...
                  new Distinct<>(
                      qc,
                      new Sort<>(qc, new GameIdsByEntities<>(qc, mockOperator, EntityType.PLAYER))),
                  null),
              EntityType.PLAYER,
              kasparovFilter,
              GameEntityJoinCondition.WHITE));
  ...
  public void gamesBySimpleAnnotatorJoinAndComplexTournamentJoin() {
    EntityQuery<Tournament> walkoverTournamentsQuery =
        new EntityQuery<>(
            db,
            EntityType.TOURNAMENT,
            null,
            new GameQuery(db, List.of(new ResultsFilter("0-0"))),
            null);
    EntityQuery<Annotator> annotatorQuery =
        new EntityQuery<>(
            db, EntityType.ANNOTATOR, List.of(new AnnotatorNameFilter("foo", true, false)));
    GameQuery gameQuery =
        new GameQuery(
            db,
            null,
            List.of(
                new GameEntityJoin<>(annotatorQuery, null),
                new GameEntityJoin<>(walkoverTournamentsQuery, null)));
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      ...
      this.assertPlanExists(
          ...
          new GameEntityHashJoin(
              ...
              new GameEntityLoopJoin<>(
                  ...
                  new AnnotatorNameFilter("foo", true, false),
                  null),
              EntityType.TOURNAMENT,
              new Distinct<>(
                  qc,
      ...
      for (QueryOperator<Game> plan : plans) {
        ...
        for (var operatorSourceClass :
            getOperatorSourceClasses(plan).collect(Collectors.toList())) {
  ...
  public void gamesByTwoComplexJoinsAndGameFilter() {
    EntityQuery<Tournament> walkoverTournamentsQuery =
        new EntityQuery<>(
            ...
            new GameQuery(db, List.of(new ResultsFilter("0-0"))),
            null);
    EntityQuery<Player> playersInWorldChQuery =
        new EntityQuery<>(
            db, EntityType.PLAYER, null, new GameQuery(db, List.of(new TournamentFilter(1))), null);
    GameQuery gameQuery =
        new GameQuery(
            db,
            null,
            List.of(
                new GameEntityJoin<>(playersInWorldChQuery, null),
                new GameEntityJoin<>(walkoverTournamentsQuery, null)));
    try (var txn = new DatabaseReadTransaction(db)) {
      QueryContext qc = new QueryContext(txn, false);
      List<QueryOperator<Game>> plans = db.queryPlanner().getGameQueryPlans(qc, gameQuery, true);
                  ...
                  new Distinct<>(
                      ...
                      new Sort<>(
                          ...
                          new EntityIdsByGames<Player>(
                              qc,
                              EntityType.PLAYER,
                              new GameTableScan(qc, new TournamentFilter(1)),
                              null))),
                  null),
              EntityType.TOURNAMENT,
              new Distinct<>(
  ...
  private Stream<Class<?>> getOperatorSourceClasses(QueryOperator<?> op) {
    return Stream.concat(
        Stream.of(op.getClass()), op.sources().stream().flatMap(this::getOperatorSourceClasses));
  }

  private <T extends IdObject> void assertPlanExists(
      List<QueryOperator<T>> plans, QueryOperator<T> expectedPlan) {
