...
public class QueryPlanComparisonTests {
  ...
  public void simpleGameQuery() {
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      ...
      List<QueryOperator<Game>> plans =
          db.queryPlanner().getGameQueryPlans(queryContext, gameQuery, true);
  ...
  public void gamesBySinglePlayer() {
    GameQuery gameQuery =
        new GameQuery(
            db, List.of(new PlayerFilter(db.getPlayer(10), null)), null, QuerySortOrder.byId(), 0);
    try (var txn = new DatabaseReadTransaction(db)) {
      QueryContext queryContext = new QueryContext(txn, false);
      List<QueryOperator<Game>> plans =
          db.queryPlanner().getGameQueryPlans(queryContext, gameQuery, true);
  ...
  public void gamesBySinglePlayerWithMatchCondition() {
    GameQuery gameQuery =
        new GameQuery(
            db,
            List.of(new PlayerFilter(db.getPlayer(10), GameEntityJoinCondition.WINNER)),
            null,
            QuerySortOrder.byId(),
            0);
    try (var txn = new DatabaseReadTransaction(db)) {
  ...
  public void gamesByPlayerNamePrefixAndTournamentDateRange() {
    EntityQuery<Player> playerQuery =
        new EntityQuery<Player>(
            db, EntityType.PLAYER, List.of(new PlayerNameFilter("K", "", true, false)));
    EntityQuery<Tournament> tournamentQuery =
        new EntityQuery<Tournament>(
            db,
            EntityType.TOURNAMENT,
            List.of(new TournamentStartDateFilter(new Date(1900, 1, 1), new Date(2000, 1, 1))));
  ...
  public void playersById() {
    EntityQuery<Player> playerQuery =
        new EntityQuery<Player>(
            db, EntityType.PLAYER, List.of(new ManualFilter<>(new int[] {10}, EntityType.PLAYER)));

    try (var txn = new DatabaseReadTransaction(db)) {
      QueryContext queryContext = new QueryContext(txn, false);
      List<QueryOperator<Player>> plans =
          db.queryPlanner().getEntityQueryPlans(queryContext, playerQuery, true);
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      QueryContext queryContext = new QueryContext(txn, false);
      List<QueryOperator<Player>> plans =
          db.queryPlanner().getEntityQueryPlans(queryContext, playerQuery, false);
  ...
  public void playersByGames() {
    ...
    EntityQuery<Player> playerQuery =
        new EntityQuery<Player>(
            db,
            EntityType.PLAYER,
            List.of(new PlayerNameFilter("K", true, false)),
            games,
  ...
  public void playersSortedByName() {
    EntityQuery<Player> playerQuery =
        new EntityQuery<>(
            db, EntityType.PLAYER, List.of(), QuerySortOrder.byPlayerDefaultIndex(), 4);
  ...
  public void tournamentsByYearAndPlace() {
    ...
    try (var txn = new DatabaseReadTransaction(db)) {
      ...
      List<QueryOperator<Tournament>> plans =
          db.queryPlanner().getEntityQueryPlans(queryContext, tournamentQuery, true);
  ...
  public void tournamentsByYearAndTitle() {
    EntityQuery<Tournament> tournamentQuery =
        new EntityQuery<Tournament>(
            db,
            EntityType.TOURNAMENT,
            List.of(new TournamentYearTitleFilter(1948, "World-ch17 Tournament", true, false)));
  ...
  public void tournamentsByGames() {
    ...
    EntityQuery<Tournament> tournamentQuery =
        new EntityQuery<Tournament>(
            db,
            EntityType.TOURNAMENT,
            List.of(new TournamentStartDateFilter(new Date(1950), Date.unset())),
            games,
            null);
  ...
  private <T extends IdObject> boolean operatorExists(
      List<QueryOperator<T>> plans, List<Class<?>> expectedClasses) {
    for (QueryOperator<T> plan : plans) {
