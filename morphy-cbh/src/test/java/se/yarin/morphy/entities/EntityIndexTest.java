...
public class EntityIndexTest {
  ...
  public void testGetDuplicateEntitiesByKeyRandomInserts() {
    // Add 1000 random entities and ensure that when fetching entities by key, we get the correct
    // amount each time.
    // Since all entities will have 1 of the 5 candidate keys, there will be lots and lots of
    // duplicates.
    int noOps = 1000;
  ...
  public void testAddMultipleEntitiesAndThenDeleteThem() {
    for (int iter = 0; iter < 100; iter++) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        index.delete(ix);
        List<Integer> actualLeft =
            index.getAll().stream().map(FooEntity::id).collect(Collectors.toList());
  ...
  public void testAddRemoveDuplicateEntities() {
    ...
    for (int ops = 0; ops < noOps; ops++) {
      ...
      index.validateStructure();
      List<String> actual =
          index.getAllOrdered().stream().map(FooEntity::key).collect(Collectors.toList());
