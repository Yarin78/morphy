...
public class FileBlobStorageTest {
  ...
  private BlobStorage createStorage() throws IOException {
    ...
    file.delete();
    FileBlobStorage storage =
        new FileBlobStorage(
            file, context, new StringBlobSizeRetriever(), Set.of(READ, WRITE, CREATE_NEW));
    // The default chunk size is much bigger; this will ensure the insert operations actually moves
    // stuff
  ...
  public void testGetBlobsFromStorageWithShortHeader() throws IOException {
    File shortheader =
        ResourceLoader.materializeStream(
            ...
            ".bin");
    FileBlobStorage storage =
        new FileBlobStorage(shortheader, context, new StringBlobSizeRetriever(), Set.of(READ));
  ...
  public void testWriteBlobsToStorageWithShortHeader() throws IOException {
    File file =
        ResourceLoader.materializeStream(
            ...
            ".bin");
    FileBlobStorage storage =
        new FileBlobStorage(file, context, new StringBlobSizeRetriever(), Set.of(READ, WRITE));
    assertEquals(10, storage.getHeader().headerSize());
  ...
  public void testRandomInserts() throws IOException {
    ...
    for (int i = 0; i < n; i++) {
      assertEquals(
          "String " + i + " mismatches", largeStrings[i], parseBlob(storage.getBlob(ofs[i])));
    }
  }
}
