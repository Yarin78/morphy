...
/**
 * Event that changes the current position in the game model. The position is specified an index in
 * the game tree traversed depth first, where 0 is the start position of the game tree.
 */
public class SetCursorEvent extends GameEvent {
  ...
  public void apply(@NotNull NavigableGameModel model) throws GameEventException {
    ...
    if (index < 0 || index >= allNodes.size()) {
      throw new GameEventException(
          ...
          model,
          String.format(
              "Tried to select move with index %d but there are only %d nodes",
              index, allNodes.size()));
    }
  ...
  public String toString() {
    return "SetCursorEvent{" + "index=" + index + '}';
  }
}
