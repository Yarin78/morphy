...
/**
 * A complete version of a chess move, containing information to generate the SAN and LAN notation
 * of the move.
 *
 * <p>Move is immutable. It's possible to construct illegal moves.
 */
public class Move {
  ...
  public Move(
      ...
      int toCol,
      int toRow,
      @NotNull Stone promotionStone) {
    this(
        fromPosition,
        Chess.coorToSqi(fromCol, fromRow),
        Chess.coorToSqi(toCol, toRow),
        promotionStone);
  }
  ...
  public Move(@NotNull Position fromPosition, int fromSqi, int toSqi, Stone promotionStone) {
    ...
    this.castles =
        Math.abs(fromSqi - toSqi) == 16 && fromPosition.stoneAt(fromSqi).toPiece() == KING;
  }
  ...
  public static Move longCastles(@NotNull Position fromPosition) {
    ...
    return new Move(
        fromPosition, Chess960.getKingSqi(sp, player), 16 + (player == Player.WHITE ? 0 : 7), true);
  }
  ...
  public static Move shortCastles(@NotNull Position fromPosition) {
    ...
    Player player = fromPosition.playerToMove();
    return new Move(
        fromPosition, Chess960.getKingSqi(sp, player), 48 + (player == Player.WHITE ? 0 : 7), true);
  }
  ...
  public boolean isEnPassant() {
    return movingPiece() == PAWN
        && Chess.deltaCol(fromSqi(), toSqi()) != 0
