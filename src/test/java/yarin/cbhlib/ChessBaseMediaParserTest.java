        ...
        Files.walk(Paths.get("/Users/yarin/chessbasemedia/mediafiles/TEXT")).forEach(filePath -> {
            if (Files.isRegularFile(filePath) && filePath.toString().endsWith(".wmv")) {
                try {
                    ...
                    ASFScriptCommandReader reader = new ASFScriptCommandReader(filePath.toFile());
                    reader.init();
                    int invalidActions = 0, totalActions = 0;
                    while (reader.hasMore()) {
                        ...
                        totalActions++;
//                        log.info(String.format("Applying action %s at %d", action.toString(), cmd.getMillis()));
                        try {
                            action.apply(gameModel);
                        } catch (ApplyActionException e) {
                            log.warn(String.format("Error applying action %s at %d", action.toString(), cmd.getMillis()));
                            invalidActions++;
                        }
                    }
                    log.info(String.format("%d actions (%d invalid)", totalActions, invalidActions));
                } catch (IOException e) {
...
//        video = "Ari Ziegler - French Defence/2.wmv";
...
//        video = "Garry Kasparov - Queens Gambit/20.wmv";
...
//        video = "Alexei Shirov - My Best Games in the Najdorf (only audio)/My best games in the Sicilian Najdorf.html/2.wmv";
        video = "Alexei Shirov - My Best Games in the Najdorf (only audio)/My best games in the Sicilian Najdorf.html/5.wmv";
//        video = "Jacob Aagaard - The Nimzoindian Defence - The easy way (only audio)/The Nimzoindian Defence - The easy way.html/6.wmv";
...
//        video = "CBM168/168!Start.html/CBM 168 Rogozenco - Classical Paulsen-Morphy.wmv";
        ...
        RecordedGame recordedGame = RecordedGame.load(new File(base + video), false);
        AnnotatedGame game = new AnnotatedGame();
        GameModel model = new GameModel(game, new GameMetaData(), game);
        recordedGame.applyActionsBetween(model, -1, recordedGame.getLastEventTime());
    }
}
