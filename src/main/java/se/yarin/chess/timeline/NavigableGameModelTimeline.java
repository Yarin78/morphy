 ...
 * The underlying {@link NavigableGameModel} can be mutated by changing the timeline cursor
 * using {@link #playTo(int)} or {@link #jumpTo(int)}.
 */
public class NavigableGameModelTimeline {
     ...
     * @return the next event timestamps, or Integer.MAX_VALUE if there
     * are no more events to be applied
     */
    ...
    /**
     * Jumps to a specific time on the timeline, updating the game model to reflect this.
     * Any events happening at exactly the specified timestamp will also be applied.
     *
     * Exactly one non-incremental {@link GameEvent} will be applied.
     *
    ...
    public void jumpTo(int timestamp) {
        ...
        if (ix < 0) {
            ...
            try {
                ...
                log.error(String.format("Failed to apply full update event %s at %s: %s",
    ...
    /**
     * Finds the latest non-incremental {@link GameEvent} that occurs
     * at or before the specified timestamp.
     * @param timestamp the latest time to find a non-incremental GameEvent
     * @return the index of a non-incremental {@link GameEvent}, or -1 if none can be found
     * in the timeline at or before the given timestamp
     */
    private int getLatestFullUpdate(int timestamp) {
        ...
        int ix = Collections.binarySearch(events,
                new TimestampedGameEvent(timestamp, new NullEvent()));
        if (ix < 0) {
    ...
    /**
     * Applies all events between {@link #getCurrentTimestamp()} (exclusive) and the
     * specified timestamp (inclusive).
     * This has better performance than {@link #jumpTo(int)} in case of small deltas
     * since only incremental updates usually needs to be applied.
     * @param timestamp the timestamp to play to. Must not be smaller than the {@link #getCurrentTimestamp()}
    ...
    public int playTo(int timestamp) {
        if (timestamp < currentTimestamp) {
            throw new IllegalArgumentException("Can't play to an earlier point in time than the current time");
        }
        ...
        while (getNextEventTimestamp() <= timestamp) {
            ...
            try {
                ...
                log.warn(String.format("Failed to apply event %s at %s: %s",
                        event.getClass().getSimpleName(),
                        formatMillis(event.getTimestamp()),
                        e.getMessage()));
            }
