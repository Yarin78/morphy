...
public class DatabaseBuilder extends GameConsumerBase {
  ...
  public void finish() {
    ...
    System.out.printf(
        "%d games added to %s in %.2f s%n", totalFoundGames, file, totalSearchTime / 1000.0);
  }
  ...
  public void accept(Game game) {
      ...
      if (game.guidingText()) {
        ...
        try {
          ...
          log.warn(
              "Failed to add game "
                  + game.id()
                  + " in the searched database in the output database",
              e);
          return;
        }
      }
    ...
    if (log.isDebugEnabled()) {
      if (game.guidingText()) {
        log.debug(
            String.format("%d: Text added to new database with id %d", game.id(), addedGame.id()));
      } else {
        log.debug(
            String.format(
                "%d: %s-%s added to new database with id %d",
                game.id(),
                game.white().getFullNameShort(),
                game.black().getFullNameShort(),
                addedGame.id()));
      }
    }
    gamesAdded++;
