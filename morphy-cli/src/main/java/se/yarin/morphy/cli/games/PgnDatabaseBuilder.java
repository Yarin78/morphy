...
public class PgnDatabaseBuilder extends GameConsumerBase {
  ...
  private static class PgnHeaderField {
    ...
    private PgnHeaderField(
        String pgnField, Function<GameModel, Object> fieldFetcher, boolean mandatory) {
  ...
  private static final PgnHeaderField[] PGN_HEADER_FIELDS = {
    ...
    new PgnHeaderField(
        "FEN", model -> model.moves().isSetupPosition() ? "not supported" : null, false),
    ...
    for (String word : s.split(" ")) {
      char lastChar =
          this.currentLine.length() > 0
              ? this.currentLine.charAt(this.currentLine.length() - 1)
              : ' ';
  ...
  private void outputLastMove(GameMovesModel.Node node, boolean showMoveNumber) {
    TextBeforeMoveAnnotation preMoveTextAnnotation =
        node.getAnnotation(TextBeforeMoveAnnotation.class);
    ...
    GraphicalSquaresAnnotation squaresAnnotation =
        node.getAnnotation(GraphicalSquaresAnnotation.class);
    if (squaresAnnotation != null) {
      textAfter
          .append("[%csl ")
          .append(
              squaresAnnotation.squares().stream()
                  .map(
                      square ->
                          annotationColorToChar(square.color()) + Chess.sqiToStr(square.sqi()))
                  .collect(Collectors.joining(",")))
          .append("]");
    }
    GraphicalArrowsAnnotation arrowsAnnotation =
        node.getAnnotation(GraphicalArrowsAnnotation.class);
    if (arrowsAnnotation != null) {
      textAfter
          .append("[%cal ")
          .append(
              arrowsAnnotation.arrows().stream()
                  .map(
                      arrow ->
                          annotationColorToChar(arrow.color())
